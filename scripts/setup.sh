...
if command -v goose &>/dev/null; then
  ...
  echo "  Install it from https://block.github.io/goose/ then re-run setup."
  if ! confirm "Continue without Goose? (you can install it later)"; then
    echo "  Setup cancelled. Install Goose then re-run."
    exit 0
  fi
  warn "Continuing without Goose."
fi
