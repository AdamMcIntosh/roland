...
export class TaskGitWorkflow {
  ...
  async onTaskComplete(task: TeamTask, startInfo?: TaskGitInfo): Promise<TaskGitInfo> {
    ...
    const prBody = buildPrBody(task, this.opts.goal, this.opts.runId, this.opts.missionUrl);
...
function buildPrBody(
  ...
  const lines = [
    ...
    '## Roland',
    `Run ID: \`${runId}\``,
    'Generated by Roland task git workflow.',
  ];
  if (missionUrl) {
    lines.push('', `Dashboard: ${missionUrl}`);
  }
  return lines.join('\n');
}
