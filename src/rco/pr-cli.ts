...
export function parsePrArgs(argv: string[]): PrCliArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === '--review-only')                          { reviewOnly = true; continue; }
    if ((a === '--branch' || a === '-b') && args[i+1]) { branch     = args[++i]; continue; }
    ...
    if (!a.startsWith('-') && /^\d+$/.test(a))         { prNumber   = Number(a); continue; }
  }
...
function getPrData(prNum: number): PrData {
  const json = execSync(
    `gh pr view ${prNum} --json number,title,body,headRefName,baseRefName,author,additions,deletions,changedFiles,files`,
    { encoding: 'utf-8', stdio: ['ignore', 'pipe', 'pipe'] },
  );
  return JSON.parse(json) as PrData;
}

function getCurrentPrNumber(): number | null {
  try {
...
function getPrDiff(prNum: number, maxChars = 8_000): string {
  try {
...
function buildPrGoal(pr: PrData, diff: string, mode: 'review' | 'fix'): string {
...
function gitCommitAndPush(prNum: number, branch?: string): void {
...
export async function runPrCli(argv: string[]): Promise<void> {
  ...
  // Synthesis to stdout
  if (!quiet) console.log(result.synthesis);
}
