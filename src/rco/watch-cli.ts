...
export function parseWatchArgs(argv: string[]): WatchCliArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if ((a === '--task' || a === '-t') && args[i + 1])    { task       = args[++i]; continue; }
    if (a === '--pattern' && args[i + 1])                  { pattern    = args[++i]; continue; }
    if (a === '--state-dir' && args[i + 1])                { stateDir   = args[++i]; continue; }
    if (a === '--agents-dir' && args[i + 1])               { agentsDir  = args[++i]; continue; }
    if ((a === '--interval' || a === '-i') && args[i + 1]) { intervalSec = Math.max(10, Number(args[++i]) || 60); continue; }
...
function gitCommitMessage(sha: string): string {
  try {
...
function gitCommitBody(sha: string): string {
  try {
...
function gitChangedFiles(fromSha: string, toSha: string): string[] {
  try {
...
function maxMtime(dir: string, pattern: string): number {
...
function printHistory(history: RunRecord[]): void {
...
export async function runWatchCli(argv: string[]): Promise<void> {
  ...
  if (once) err(`  ${c.dim('Once mode — will exit after first run')}`);
  err('  ' + '═'.repeat(COLS - 2));
  err('');
  ...
  // ── Main loop ──────────────────────────────────────────────────────────────
  while (true) {
    await new Promise<void>((r) => setTimeout(r, intervalSec * 1000));
    ...
    if (isGitMode) {
      ...
      if (currentMtime > lastMtime) {
        ...
        if (!goal) goal = `Files matching "${pattern}" changed. Review changes, fix issues, and run tests.`;
    ...
    try {
      ...
      if (!quiet) {
        ...
        if (result.synthesis.length > 600) err(`  ${c.dim('…(full synthesis above)')}`);
        err('');
      }
