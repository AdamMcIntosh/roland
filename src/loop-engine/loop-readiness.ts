...
function check(id: string, ok: boolean, message: string, severity: ReadinessSeverity = ok ? 'info' : 'error'): ReadinessCheck {
...
export function runLoopReadinessCheck(options: { configPath?: string } = {}): LoopReadinessReport {
  ...
  checks.push(
    check(
      'loop_config',
      true,
      `loop_engine loaded — default_dispatch=${loopCfg.defaultDispatch ?? 'cursor_sdk'} use_pm_team=${loopCfg.usePmTeam ?? false}`,
    ),
  );
  ...
  checks.push(
    check(
      'cursor_sdk_key',
      validation.cursorSdkAvailable || validation.defaultDispatch === 'direct',
      validation.cursorSdkAvailable
        ...
        : validation.defaultDispatch === 'direct'
          ...
          : 'CURSOR_API_KEY missing — SDK default will fall back to direct provider at runtime',
      validation.cursorSdkAvailable ? 'info' : 'warn',
    ),
  );

  const coreRoles = ['pm', 'coding', 'critic', 'verifier'] as const;
  for (const role of coreRoles) {
    ...
    checks.push(
      check(
        `dispatch_${role}`,
        Boolean(d.model),
        `${role}: ${d.method} → ${d.displayLabel} (${d.reason})`,
      ),
    );
  }
  ...
  checks.push(
    check(
      ...
      `${templateList.length} loop template(s) loaded`,
    ),
  );

  for (const err of templates.getLoadErrors()) {
    checks.push(
      check('template_load', false, `Malformed template ${err.file}: ${err.message}`),
    );
  }
  ...
  if (defaultTpl) {
    ...
    checks.push(
      check(
        'pm_integration_default',
        true,
        `Default template PM mode: ${pmStatus.enabled ? 'PM-Enhanced' : 'Pure ClosedLoop'} — ${pmStatus.reason}`,
      ),
    );
  }
  ...
  for (const issue of lintErrors.slice(0, 5)) {
    checks.push(
      check(`lint_${issue.code}_${issue.template}`, false, `[${issue.template}] ${issue.message}`),
    );
  }
  for (const issue of lintWarns.slice(0, 3)) {
    checks.push(
      check(`lint_${issue.code}_${issue.template}`, true, `[${issue.template}] ${issue.message}`, 'warn'),
    );
  }
...
export function formatLoopReadinessReport(report: LoopReadinessReport): string {
  const lines: string[] = [
    '═══════════════════════════════════════════════════════════',
  ...
  if (errors.length) {
    lines.push('Errors:');
    for (const c of errors) lines.push(`  ✗ [${c.id}] ${c.message}`);
    lines.push('');
  }
  if (warns.length) {
    lines.push('Warnings:');
    for (const c of warns) lines.push(`  ⚠ [${c.id}] ${c.message}`);
    lines.push('');
  }
  lines.push('Checks:');
  ...
  if (infos.length) {
    lines.push('');
    lines.push('Info:');
    for (const c of infos.slice(0, 8)) lines.push(`  · ${c.message}`);
  }
