 ...
 *
 * Lifecycle: PLAN → ACT → VERIFY → CRITIQUE → RETRY → ESCALATE (optional) → next iteration or complete.
 */
export class ClosedLoop {
  ...
  constructor(opts: ClosedLoopOptions) {
    ...
    console.error(
      `[Loop][closed-loop] harness ready template="${this.template.name}" ` +
        `phases=${this.template.phases.map((p) => p.phase).join('→')}`,
    );
  }

  /** Run the full closed loop until complete, escalate, fail, or timeout. */
  ...
  private persistFormattedPr(state: LoopState, status: LoopRunStatus): FormattedPr {
    ...
    try {
      ...
      fs.writeFileSync(
        prPath,
        JSON.stringify({ ...formatted, status, iteration: state.iteration, at: Date.now() }, null, 2),
        'utf-8',
      );
    } catch {
...
/**
 * ## Component Complete
 * ClosedLoop orchestrates Plan→Act→Verify→Critique→Retry→Escalate cycles with EvaluationGate,
 * dynamic specialist spawning, checkpoint recovery, and PR formatter integration.
 */
