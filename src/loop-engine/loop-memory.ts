...
export interface ReflectionEntry {
  ...
  at: number;
  content: string;
  /** Structured reflection fields when generated by ReflectionPhaseHandler. */
  structured?: StructuredReflection;
}

/** Parsed sections from the structured reflection format. */
...
export interface SpecProgressSnapshot extends SpecProgress {
  recordedAt: number;
}
...
export interface LoopDiskState {
  ...
  /** Latest spec/checklist progress when template defines specFile or checklistPath. */
  specProgress?: SpecProgressSnapshot;
  specProgressHistory?: SpecProgressSnapshot[];
}
...
export class LoopMemory {
  ...
  getState(): LoopDiskState {
    return {
      ...
      specProgress: this.diskState.specProgress ? { ...this.diskState.specProgress } : undefined,
      specProgressHistory: this.diskState.specProgressHistory?.map((s) => ({ ...s })),
    };
  }
  ...
  appendReflection(iteration: number, content: string, structured?: StructuredReflection): ReflectionEntry {
    const entry: ReflectionEntry = { iteration, at: Date.now(), content, structured };
  ...
  /** Record spec/checklist progress snapshot for dashboard and exit conditions. */
  recordSpecProgress(progress: SpecProgress): SpecProgressSnapshot {
    const snapshot: SpecProgressSnapshot = { ...progress, recordedAt: Date.now() };
    this.diskState.specProgress = snapshot;
    if (!this.diskState.specProgressHistory) {
      this.diskState.specProgressHistory = [];
    }
    this.diskState.specProgressHistory.push(snapshot);
    if (this.diskState.specProgressHistory.length > 32) {
      this.diskState.specProgressHistory.shift();
    }
    this.touch();
    return snapshot;
  }

  /** Return the most recent reflection entry, if any. */
  getLatestReflection(): ReflectionEntry | undefined {
    return this.diskState.reflections.at(-1);
  }

  /** Full reflection.md contents for feeding into subsequent iteration context. */
  getReflectionContext(): string {
    return this.readReflectionMd();
  }
 ...
 * LoopMemory persists reflections, spec progress, exit-condition tracking, and between-iteration artifacts
