...
function humanAge(sinceMs?: number): string {
  if (!sinceMs) return '';
...
export function renderStandup(ctx: TeamContext): string {
  const s = ctx.summary;
  ...
  const byKind = (k: AttentionItem['kind']) => ctx.needsAttention.filter((a) => a.kind === k);
  ...
  if (reviews.length > 0) {
    ...
    for (const a of reviews) lines.push(`- ${a.reason}\n  - \`${a.action}\``);
    lines.push('');
  }
  ...
  if (stalled.length > 0) {
    ...
    for (const a of stalled) lines.push(`- ${a.reason}`);
    lines.push('');
  }
  ...
  if (ready.length > 0) {
    ...
    for (const a of ready) lines.push(`- ${a.reason}\n  - \`${a.action}\``);
    lines.push('');
  }

  lines.push(
    `**Board:** open ${s.open} · in_progress ${s.in_progress} · blocked ${s.blocked} · in_review ${s.in_review} · done ${s.done}`
  );
  const u = ctx.usage;
  lines.push(
    `**Usage:** ${tokens(u.totalInputTokens + u.totalOutputTokens)} tokens across ${Object.keys(u.byEngineer).length} engineer(s) · ${u.totalRequests} request(s) _(Cursor subscription)_`
  );
...
export function renderBoard(tasks: TaskView[]): string {
  const cols: Array<[string, TaskView['status']]> = [
    ['Open', 'open'],
  ...
  for (const [label, status] of cols) {
    ...
    lines.push(`\n### ${label} (${inCol.length})`);
    if (inCol.length === 0) lines.push('- _(none)_');
...
export function renderUsage(usage: TeamUsage): string {
...
export function renderDispatch(packet: DispatchPacket): string {
  return [
...
export function renderTimeline(events: PMEvent[]): string {
