...
export interface QualityRecord {
  ...
  model: string;
  provider: string;
  task_type: string;        // from complexity classifier tier
  complexity_tier: string;  // local, simple, medium, complex
  signal: 'accept' | 'retry' | 'reject' | 'manual_fix';
  retry_model?: string;     // what model they retried with
}
...
export interface ModelQuality {
  ...
  manual_fix_rate: number;  // 0-1
  best_task_types: string[];   // tiers where accept_rate > 0.7 (min 5 signals)
  worst_task_types: string[];  // tiers where accept_rate < 0.4 (min 5 signals)
}
...
export class QualityTracker {
   ...
   */
  getRecommendation(tier: string): Array<{ model: string; score: number }> {
    ...
    return models
      .map(model => {
        ...
        if (modelRecords.length <= 10) return null;
  ...
  private computeQuality(model: string): ModelQuality {
    ...
    const countSignal = (s: string) =>
      modelRecords.filter(r => r.signal === s).length;
    ...
    const best_task_types: string[] = [];
    const worst_task_types: string[] = [];

    for (const tier of tiers) {
      const tierRecords = modelRecords.filter(r => r.complexity_tier === tier);
      if (tierRecords.length < 5) continue;
...
let globalQualityTracker: QualityTracker | null = null;

/**
 * Get the global QualityTracker instance.
 * Returns null if not yet initialized.
 */
export function getGlobalQualityTracker(): QualityTracker | null {
 ...
 */
export function initializeQualityTracker(projectRoot: string): QualityTracker {
