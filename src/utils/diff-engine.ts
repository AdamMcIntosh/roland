 ...
 */
function greedyMatchByHash(a: string[], b: string[]): Array<[number, number]> {
 ...
 */
function toMarkdown(hunks: DiffHunk[], filename: string): string {
  if (hunks.length === 0) {
    return '```diff\n(no changes)\n```';
  }
 ...
 */
function toHtml(hunks: DiffHunk[], filename: string, additions: number, deletions: number): string {
  if (hunks.length === 0) {
