 ...
 */
export function isSimpleTui(): boolean {
  // 1. Explicit opt-in
  if (process.env.ROLAND_SIMPLE_TUI === '1') return true;

  // 2. Dumb terminal
  if (process.env.TERM === 'dumb') return true;

  // 3. Very narrow terminal
  const termCols = (process.stdout as NodeJS.WriteStream & { columns?: number }).columns ?? 0;
  if (termCols > 0 && termCols < 60) return true;
  ...
  if (isSSH && !hasTCColor) return true;
...
export class SimpleTuiRenderer {
  ...
  private readonly printedTaskIds = new Set<string>();
  ...
  private printDelta(state: RunState): void {
    const now = Date.now();
    const C   = cols();
