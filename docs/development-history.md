...
## Phase 2 — Persistence, Validation, and Dashboard

**Focus**: Session persistence, schema validation, Tauri dashboard.
...
- **Schemas** (`src/schemas.ts`): Zod validation for prompts, responses, state
...
- 19 Phase 2 integration tests covering persistence, schemas, export, dashboard, prompts
...
- **Customization**: `generateAndSaveCustomAgent` creates YAML from a prompt description
...
- **Packaging**: `npm run build-npm`, `npm run build-tauri`
...
- **Telemetry** (`src/telemetry.ts`): Opt-in Sentry with consent management
