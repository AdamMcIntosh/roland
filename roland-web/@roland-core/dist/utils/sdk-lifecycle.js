...
export const SHELL_EXEC_CLOSE_WARNING_RE = /\[shell-exec\]\s*Close event did not fire within \d+ms/i;
export function isShellExecCloseWarning(text) {
...
export function installShellExecWarningSilencer() {
    if (shellExecSilencerInstalled)
        return;
    ...
    console.warn = (...args) => {
        if (isShellExecCloseWarning(argsToText(args)))
            return;
        origWarn(...args);
    };
    console.error = (...args) => {
...
export function createShellExecStderrFilter() {
...
export function configureSdkProcessLimits() {
    if (processLimitsConfigured)
        return;
...
export function getDefaultSdkSettleMs() {
    return SDK_SETTLE_MS;
}
/** Default heavy-task settle ms (env-overridable). */
export function getHeavySdkSettleMs() {
    return SDK_HEAVY_SETTLE_MS;
}
const SHELL_EXEC_HEAVY_AGENTS = /test-author|test-executor|tdd-guide|build-fixer|test-executor|qa-tester/i;
const SHELL_EXEC_HEAVY_CONTEXT = /\b(npm test|npm run test|vitest|jest|pytest|cargo test|go test|dotnet test|make test|shell|exec|compile|build)\b/i;
/** Local Agent.create options — all agents use silent shell children (no visible windows). */
export function resolveSdkAgentLocalOptions(_agentName, base) {
    return {
        ...base,
 ...
 */
export function resolveSdkSettleMs(agentName, taskContext) {
...
export class SdkAgentTimeoutError extends Error {
    ...
    constructor(agentName, timeoutMs) {
        super(`Agent "${agentName}" timed out after ${(timeoutMs / 60_000).toFixed(0)} min. ` +
...
function killProcessAggressive(pid) {
 ...
 */
function collectChildPids(root, seen = new Set(), depth = 0) {
 ...
 */
export async function forceKillAfterSettle(agent, opts) {
    if (!agent)
        return { forced: false, killedPids: [] };
    ...
    if (killedPids.length > 0) {
        ...
        console.error(`[Roland] Force cleanup${who}: SIGKILL${groupNote} on ${killedPids.length} lingering shell child process(es) — pids=${killedPids.join(', ')}`);
 ...
 */
export async function waitForRunTerminal(run, timeoutMs = RUN_TERMINAL_WAIT_MS) {
    if (!run)
        return true;
    ...
    const drainWithTimeout = async (budgetMs) => {
        if (!run.wait || budgetMs <= 0)
            return;
        try {
            await Promise.race([
                run.wait().then(() => {
 ...
 */
export async function cancelSdkRun(run, opts) {
    if (!run?.cancel)
        return;
    if (isTerminalRunStatus(run.status) && !opts?.force)
        return;
    try {
        await run.cancel();
    }
 ...
 */
export async function settleSdkRun(run, opts) {
    if (!run)
        return;
    await waitForRunTerminal(run);
 ...
 */
export async function waitForSdkRun(run, options = {}) {
 ...
 */
export async function disposeSdkAgent(agent) {
    if (!agent)
        return;
    try {
...
export async function cleanupSdkSession(agent, run, opts) {
...
//# sourceMappingURL=sdk-lifecycle.js.map