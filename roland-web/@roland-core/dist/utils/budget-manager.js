...
export class BudgetManager {
    static budgetFile = '.cache/budget.json';
    static config = {
        maxBudget: 85.00,
        currentSpending: 0,
    ...
    static checkMonthlyReset() {
        ...
        if (currentMonth > lastResetMonth && now.getDate() >= resetDay) {
            ...
            this.config.resetDate = now.getTime();
            this.save();
            logger.info(`[Budget] Monthly auto-reset: $${previousSpending.toFixed(4)} → $0.00 ` +
    ...
    static configureFromAppConfig(options) {
        ...
        if (changed) {
            this.save();
            logger.info(`[Budget] Configured: $${this.config.maxBudget.toFixed(2)}/month, ` +
     ...
     */
    static checkBudget(estimatedCost) {
        this.initialize();
        ...
        if (newTotal > this.config.maxBudget) {
            throw new Error(`Budget exceeded! Cost: $${estimatedCost.toFixed(6)}, ` +
        ...
        if (usagePercent >= this.config.warningThreshold &&
            ...
            logger.warn(`[Budget] WARNING: ${(usagePercent * 100).toFixed(0)}% of budget used ` +
