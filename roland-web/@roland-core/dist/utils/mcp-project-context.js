...
function hasExplicitArg(args, key) {
    const v = args?.[key];
    return typeof v === 'string' && v.trim().length > 0;
}
/** Derive project root when only a `.roland` path is known. */
export function deriveProjectRootFromStateDir(stateDir) {
    const resolved = path.resolve(stateDir);
    if (path.basename(resolved) === '.roland')
        return path.dirname(resolved);
    if (fs.existsSync(path.join(resolved, '.roland')))
        return resolved;
    return path.dirname(resolved);
}
function normalizeStateDir(projectRoot, rawState) {
...
export function resolveMcpProjectContext(args) {
    ...
    const explicitStateArg = hasExplicitArg(args, 'state_dir');
    ...
    if (explicitProject) {
        ...
        projectRoot = path.resolve(pickString(process.env['ROLAND_PROJECT_ROOT'], process.env['ROLAND_ROOT']));
    }
...
export function resolveMissionProjectRoot(fallbackCwd) {
...
export function resolveMissionProjectRootFromState(stateDir) {
    const fromEnv = pickString(process.env['ROLAND_PROJECT_ROOT'], process.env['ROLAND_ROOT']);
    if (fromEnv)
        return path.resolve(fromEnv);
    return deriveProjectRootFromStateDir(stateDir);
}
const ROLAND_ENV_KEYS = ['ROLAND_PROJECT_ROOT', 'ROLAND_ROOT', 'ROLAND_STATE_DIR'];
/** Build env overrides for a project context without mutating process.env. */
export function scopedProjectEnv(ctx, base = process.env) {
    return {
        ...base,
 ...
 */
export function applyMcpProjectEnv(ctx) {
...
export async function withProjectContext(ctx, fn) {
    const prior = {};
...
export function chdirToProject(ctx) {
    try {
 ...
 */
export function ensureMissionProjectContext(ctx) {
    applyMcpProjectEnv(ctx);
    chdirToProject(ctx);
 ...
 */
//# sourceMappingURL=mcp-project-context.js.map