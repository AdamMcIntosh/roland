...
export const DEFAULT_CONTEXT_GATHERING_CONFIG = {
    enabled: true,
    max_files: 15,
 ...
 */
export function listProjectFiles(excludePatterns = []) {
    try {
        ...
        logger.warn(`[FileGatherer] git ls-files failed: ${err.message}`);
        return [];
    }
}
/**
 ...
 */
export async function askLlmForRelevantFiles(task, candidates, config) {
...
Respond with ONLY a JSON array like: ["path/to/file1.ts", "path/to/file2.ts"]`;
    try {
        ...
        const response = await httpsPost('https://openrouter.ai/api/v1/chat/completions', body, {
        ...
        const valid = selectedFiles.filter((f) => typeof f === 'string' && candidateSet.has(f));
        logger.info(`[FileGatherer] LLM selected ${valid.length} files from ${candidates.length} candidates`);
        return valid.length > 0 ? valid : null;
    }
    catch (err) {
        logger.warn(`[FileGatherer] LLM file selection failed: ${err.message}`);
        return null;
    }
}
/**
 ...
 */
export async function selectRelevantFiles(task, config = DEFAULT_CONTEXT_GATHERING_CONFIG) {
 ...
 */
export function bundleFileContents(files, maxBytes = DEFAULT_CONTEXT_GATHERING_CONFIG.max_bytes) {
    const bundle = {
        files: [],
        totalBytes: 0,
        truncated: false,
    ...
    for (const filePath of files) {
        try {
            ...
            if (bundle.totalBytes + sizeBytes > maxBytes) {
                ...
                if (bundle.totalBytes > maxBytes * 0.8)
                    break;
                continue;
            }
    ...
    logger.info(`[FileGatherer] Bundled ${bundle.files.length} files (${(bundle.totalBytes / 1024).toFixed(1)}KB)` +
 ...
 */
export function formatBundleAsMarkdown(bundle) {
...
//# sourceMappingURL=file-gatherer.js.map