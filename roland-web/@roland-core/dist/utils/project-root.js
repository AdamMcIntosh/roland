...
export function resolveRolandInstallRoot(fromUrl) {
...
export function resolveProjectRoot(startDir = process.cwd()) {
    for (const key of ['ROLAND_PROJECT_ROOT', 'ROLAND_ROOT']) {
        ...
        if (fs.existsSync(resolved))
            return resolved;
        process.stderr.write(`[roland] Warning: ${key}="${val}" does not exist — continuing search\n`);
    }
    ...
    while (true) {
        if (fs.existsSync(path.join(dir, '.roland')))
            return dir;
...
export function bootstrapRolandEnv(opts) {
    ...
    const hasProjectOverride = Boolean(process.env.ROLAND_PROJECT_ROOT?.trim()) ||
