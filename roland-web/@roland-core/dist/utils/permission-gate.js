...
export function readPermissions(projectRoot) {
...
export function scaffoldPermissionsFile(projectRoot) {
    const filePath = path.join(projectRoot, PERMISSIONS_FILE);
    if (fs.existsSync(filePath))
        return false;
 ...
 */
export function buildPermissionBlock(policy) {
    const lines = [];
    const hasRestrictions = policy.allow_shell === false ||
    ...
    if (!hasRestrictions && !policy.extra_instructions)
        return '';
 ...
 */
export function getPermissionBlock(projectRoot) {
