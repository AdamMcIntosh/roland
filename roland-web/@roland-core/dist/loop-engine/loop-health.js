...
import { LoopObservability, computeLoopMetrics, summarizeHistory, LOOP_METRICS_FILE, LOOP_HISTORY_FILE, } from './loop-observability.js';
...
function fileExists(stateDir, name) {
    return fs.existsSync(path.join(stateDir, name));
}
function readBlackboardEntries(stateDir) {
    try {
...
function readClosedLoopPr(stateDir) {
    try {
...
function extractSpecialistSpawns(entries) {
    ...
    for (const entry of entries) {
        if (!entry.tags?.includes('spawn') || !entry.tags?.includes('loop'))
            continue;
...
export function buildLoopHealthReport(stateDir) {
    ...
    const metrics = storedMetrics ?? (loopState ? computeLoopMetrics(loopState) : null);
    const diagnostics = [];
    let status = 'idle';
    if (loopState) {
        if (loopState.status === 'escalated') {
            ...
            if (!supervisorAlive)
                diagnostics.push('Loop state running but supervisor not alive');
        }
    ...
    const canResume = Boolean(runState?.hitlPaused || (loopState?.status === 'escalated' && supervisorAlive));
    ...
    const phaseHistory = loopState?.phaseHistory?.slice(-24).map((t) => ({
        phase: t.phase,
        success: t.success,
    ...
    const exitConditions = loopState?.exitConditionStatus?.map((s) => ({
        id: s.id,
        type: s.type,
    ...
    const configuredPm = template
        ...
        : { enabled: false, reason: 'no template', source: 'disabled' };
    const pmIntegration = {
    ...
    const loopSummary = isComplete || loopState
        ? {
            ...
            confidence: lastVerification && 'confidence' in lastVerification
    ...
    return {
        ...
        loop: {
            ...
            lastCritiqueDecision: loopState?.lastCritique?.retryDecision ?? runState?.lastCritique?.retryDecision ?? null,
            confidence: lastVerification && 'confidence' in lastVerification
                ? (lastVerification.confidence ?? null)
                : null,
            verificationAccepted: lastVerification && 'accepted' in lastVerification
        ...
        loopMemory: memoryState
            ? {
                ...
                lastReflection: memoryState.reflections.at(-1)?.content.slice(0, 200) ?? null,
