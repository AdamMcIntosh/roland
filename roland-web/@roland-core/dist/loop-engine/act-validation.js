...
export function inferExpectedDeliverables(goal) {
    ...
    for (const m of goal.matchAll(/\b([a-zA-Z0-9_/-]+\.(?:ts|tsx|js|jsx|json|md|yaml|yml))\b/g)) {
        ...
        if (!base || base.startsWith('.') || FALSE_FILE_POSITIVE.test(base))
            continue;
        expected.add(base);
    }
...
function diffAgainstBaseline(cwd, baseline) {
 ...
 */
export function validateActExecution(opts) {
    ...
    if (missingDeliverables.length > 0) {
        return {
            ...
            missingDeliverables,
            message: `Act phase incomplete — missing deliverables: ${missingDeliverables.join(', ')}. ` +
    ...
    if (!hasChanges && isGreenfieldGoal(opts.goal)) {
        return {
            ...
            message: `Act phase no-op — no files created or modified in ${cwd}. ` +
    ...
    if (!hasChanges) {
        ...
        if (outputLen > 80) {
            return {
                ...
                filesModified: [],
                missingDeliverables: [],
                message: 'Act phase no-op — agent produced output but no files changed on disk. ' +
    ...
    if (meaningfulCreated.length)
        parts.push(`created: ${meaningfulCreated.slice(0, 8).join(', ')}`);
    return {
        ok: true,
        filesCreated: meaningfulCreated,
        filesModified: meaningfulModified,
