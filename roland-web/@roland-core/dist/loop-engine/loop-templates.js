...
function mapSpawnConditions(raw) {
    if (!raw)
        return undefined;
    return {
...
function mapSpecialistSpawns(raw) {
    if (!raw?.length)
        return undefined;
    return raw.map((s) => ({
        role: s.role,
...
export function lintLoopTemplate(template) {
    const issues = [];
    ...
    if (template.betweenIterations) {
        const betweenBlob = typeof template.betweenIterations === 'string'
        ...
        if (typeof template.betweenIterations === 'object' &&
...
export function lintAllLoopTemplates(templates) {
    const issues = [];
...
export function summarizeTemplateSpawns(template) {
    const parts = [];
    for (const phase of template.phases) {
        if (!phase.specialistSpawns?.length)
            continue;
...
function toListEntry(template) {
    return {
        ...
        isCoreGeneric: CORE_GENERIC_TEMPLATES.includes(template.name),
...
export class LoopTemplates {
    ...
    cache = null;
    loadErrors = [];
    constructor(dir = LoopTemplates.resolveLoopsDir()) {
        this.dir = dir;
    }
    list() {
    ...
    listDetailed() {
        return Array.from(this.load().values())
            .map(toListEntry)
            .sort((a, b) => {
            if (a.isCoreGeneric !== b.isCoreGeneric)
                return a.isCoreGeneric ? -1 : 1;
            if (Boolean(a.deprecated) !== Boolean(b.deprecated))
    ...
    static resolveLoopsDir() {
        ...
        try {
            ...
            if (fs.existsSync(distLoops))
                return distLoops;
            const srcLoops = path.join(rootDir, 'recipes', 'loops');
            if (fs.existsSync(srcLoops))
                return srcLoops;
        }
        catch {
            // fall through
