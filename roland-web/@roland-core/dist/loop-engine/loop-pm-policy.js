 ...
 */
export function resolvePmIntegrationStatus(template, opts = {}) {
    ...
    const hasAlways = template.pmPlan === 'always' ||
    ...
    if (!globalOptIn && !templateOptIn) {
        const hasAuto = template.pmPlan === 'auto' ||
            template.pmAct === 'auto' ||
            template.phases.some((p) => p.pmTeam === 'auto');
        if (hasAuto) {
            return {
                enabled: false,
                reason: 'pm_plan/pm_act auto present but use_pm_team not enabled — pure ClosedLoop ' +
...
export function isLoopPmTeamEnabled(template, opts = {}) {
...
export function formatPmIntegrationLabel(status) {
...
export function logPmIntegrationMode(status, templateName) {
    ...
    if (status.enabled) {
        console.error('[Loop] [DEPRECATED] Legacy PM Team will delegate Plan/Act to team-orchestrator (pmSlice). ' +
            'Prefer Pure ClosedLoop (use_pm_team: false).');
    }
    else {
        console.error('[Loop] @roland / Pure ClosedLoop — lightweight Plan/Act · PACVRE verify/critique/reflect in harness.');
    }
}
/**
 ...
 * ```
 */
//# sourceMappingURL=loop-pm-policy.js.map