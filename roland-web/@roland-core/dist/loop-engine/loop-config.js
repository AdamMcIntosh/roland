...
function commandForStrategyType(type) {
...
function normaliseVerification(raw) {
    if (!raw)
        return DEFAULT_CONFIG.verification;
    return {
...
function normaliseCritique(raw) {
    if (!raw)
        return DEFAULT_CONFIG.critique;
    return {
        maxRetries: raw.max_retries ?? DEFAULT_CONFIG.critique?.maxRetries ?? DEFAULT_MAX_RETRIES,
        escalationThreshold: raw.escalation_threshold ??
...
function normaliseRetry(raw) {
    if (!raw?.exponential_backoff)
...
export function resolveCritiqueThresholds(template, opts = {}) {
    ...
    const baseEscalation = template.escalationThreshold ??
        cfg.critique?.escalationThreshold ??
        DEFAULT_ESCALATION_THRESHOLD;
    if (testMode) {
        return {
            maxRetries: template.testModeMaxRetries ??
                cfg.critique?.testMode?.maxRetries ??
                baseMaxRetries + 2,
            escalationThreshold: template.testModeEscalationThreshold ??
