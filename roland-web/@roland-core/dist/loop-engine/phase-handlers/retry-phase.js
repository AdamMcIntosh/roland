...
function extractFocusTargets(ctx) {
...
export class RetryPhaseHandler {
    phase = Phase.Retry;
    opts;
    constructor(opts = {}) {
        this.opts = opts;
    ...
    async execute(ctx) {
        ...
        if (shouldEscalateToHuman(escalationCtx)) {
            ...
            console.error(`[Loop][retry] escalation retryCount=${ctx.state.retryCount} maxRetries=${maxRetries} iteration=${ctx.iteration}`);
            ctx.blackboard.post({
                type: 'decision',
        ...
        const backoffEnabled = this.opts.backoffEnabled ?? cfg.retry?.exponentialBackoff?.enabled ?? false;
        ...
        if (backoffEnabled && !this.opts.skipDelay) {
            ...
            console.error(`[Loop][retry] exponential backoff ${backoffMs}ms before iteration ${ctx.iteration + 1} ` +
                `(retryCount=${ctx.state.retryCount})`);
            await sleep(backoffMs);
        }
        const strategyLabel = strategy === 'focused'
            ? `focused retry on ${focusAreas.join(', ')}`
            : 'full iteration retry';
        const fileHint = failedFiles.length > 0 ? ` · targets: ${failedFiles.slice(0, 3).join(', ')}` : '';
        const summary = `Retry attempt ${retryNum} (${strategyLabel}) scheduled for iteration ${ctx.iteration + 1}` +
            (backoffMs > 0 ? ` · backoff ${backoffMs}ms` : '') +
            fileHint;
        console.error(`[Loop][retry] strategy=${strategy} retryNum=${retryNum} iteration=${ctx.iteration} ` +
...
function countConsecutiveVerifyFailures(ctx) {
    ...
    if (count === 0 && ctx.state.lastVerification && !ctx.state.lastVerification.pass)
        count = 1;
    return count;
}
//# sourceMappingURL=retry-phase.js.map