...
export class ReflectionPhaseHandler {
    opts;
    phase = Phase.Reflect;
    constructor(opts = {}) {
        this.opts = opts;
    ...
    async execute(ctx) {
        ...
        ctx.commandBoard?.appendBullet('Key Decisions', `[REFLECT] Iteration ${ctx.iteration}: ${content.split('\n')[0]?.slice(0, 120) ?? 'learnings recorded'}`);
        ctx.blackboard.post({
            type: 'artifact',
...
function buildReflectionContent(ctx) {
    const lines = [];
...
//# sourceMappingURL=reflection-phase.js.map