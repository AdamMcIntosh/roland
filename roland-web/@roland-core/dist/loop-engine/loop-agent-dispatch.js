...
async function callSdkAgent(agentRole, prompt, cwd) {
    ...
    try {
        agent = await Agent.create({
            ...
            model: { id: modelId },
            name: agentRole,
            local: resolveSdkAgentLocalOptions(agentRole, { cwd, settingSources: ['project'] }),
        });
...
export async function dispatchLoopPhaseAgent(opts) {
    ...
    console.error(`[Loop][agent] Dispatch ${opts.phase} iter=${opts.iteration} role=${role} ` +
    ...
    try {
        ...
        if (opts.phase === 'act' && actBaseline) {
            ...
            opts.commandBoard?.appendBullet('Open Intel', validation.ok
        ...
        if (hadBlocker && signals.blockers.length > 0) {
            for (const blocker of signals.blockers) {
                ...
                opts.commandBoard?.appendBullet('Open Intel', `[BLOCKER] ${role} loop-${opts.phase}: ${blocker.description.slice(0, 160)}`);
            }
        }
        opts.blackboard.post({
            type: 'result',
