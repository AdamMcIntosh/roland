...
export function saveLoopCheckpoint(stateDir, phase, state, opts = {}) {
    ...
    try {
        ...
        console.error(`[Loop][checkpoint] saved phase=${phase} iter=${state.iteration} retry=${state.retryCount}`);
    }
    catch {
        // Non-fatal — loop continues from in-memory state.
    }
}
export function readLoopCheckpoint(stateDir) {
    try {
...
export function clearLoopCheckpoint(stateDir) {
    try {
 ...
 */
export function tryRecoverLoopState(stateDir) {
    const checkpoint = readLoopCheckpoint(stateDir);
    if (checkpoint?.state && checkpoint.state.status === 'running') {
        console.error(`[Loop][recovery] restored from checkpoint phase=${checkpoint.phase} ` +
    ...
    if (loopState && loopState.status === 'running') {
        console.error(`[Loop][recovery] restored from loop-state.json phase=${loopState.currentPhase} ` +
