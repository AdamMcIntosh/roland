...
import { createDefaultHandlers, RetryPhaseHandler, ReflectionPhaseHandler, } from './phase-handlers/index.js';
...
export class LoopEngine {
    ...
    constructor(opts) {
        ...
        if (opts.loopMemory && !opts.handlers) {
            this.handlers.set(P.Reflect, new ReflectionPhaseHandler({ memory: opts.loopMemory }));
        }
        ...
        if (opts.recoverOnStart !== false) {
            ...
            if (recovery.recovered && recovery.state) {
                ...
                this.commandBoard?.appendBullet('Key Decisions', `[LOOP] Recovered from ${recovery.source} at phase ${recovery.phase} (iter ${recovery.state.iteration})`);
            }
            else {
                this.store = LoopStateStore.loadOrCreate(opts.stateDir, opts.template.name, opts.goal, firstPhase, Boolean(opts.resumeFromState));
            }
        }
        else {
            this.store = LoopStateStore.loadOrCreate(opts.stateDir, opts.template.name, opts.goal, firstPhase, false);
        }
        console.error(`[Loop][engine] template="${opts.template.name}" maxIterations=${opts.template.maxIterations ?? 1} ` +
    ...
    async runFullLoop(context = {}) {
        ...
        for (let iter = startIter; iter <= maxIter; iter++) {
            ...
            iterationsRun++;
            console.error(`[Loop][engine] iteration ${iter}/${maxIter} retryCount=${this.store.get().retryCount}`);
            ...
            if (!iterationOutcome.shouldRetryLoop) {
                if (iter < maxIter) {
                    console.error(`[Loop][engine] exit conditions unmet — self-pacing to iteration ${iter + 1}/${maxIter}`);
                    continue;
                }
                break;
            }
            ...
            if (this.store.get().retryCount >= maxRetries) {
                console.error(`[Loop][engine] retry budget exhausted retryCount=${this.store.get().retryCount} maxRetries=${maxRetries}`);
            ...
            console.error(`[Loop][engine] scheduling next iteration after retry increment retryCount=${this.store.get().retryCount}`);
        }
        this.store.setStatus('completed');
        this.emitState();
        this.observability.persistMetrics(this.store.get());
    ...
    async runIterationPhases(iter, context) {
        let phasesCompleted = 0;
        ...
        for (const phaseConfig of this.template.phases) {
            ...
            if (phaseConfig.optional && phaseConfig.phase === P.Escalate) {
                const pendingEscalation = shouldRetryLoop &&
    ...
    buildBetweenIterationsOpts(hook, iter, hookVars) {
        return {
            hook,
            iteration: iter,
            cwd: this.cwd,
    ...
    async runPhaseAfterHook(phaseConfig, iter) {
        ...
        const result = await runBetweenIterations(this.buildBetweenIterationsOpts(hook, iter, {
            goal: this.goal,
        ...
        if (result.fatal) {
            console.error(`[Loop][engine] phase after-hook failed exit_on_failure phase=${phaseConfig.phase} iter=${iter}`);
            this.store.setStatus('failed');
        }
    }
    /** Between-iterations check, reflection, and exit condition evaluation. */
    async runPostIterationHooks(iter) {
        let phasesCompleted = 0;
        let lastBetweenRun;
        const betweenHook = resolveBetweenIterationsHook(this.template);
        if (betweenHook && this.loopMemory) {
            const requireApproval = betweenHook.gitCommit?.requireApproval && !betweenHook.gitCommit.dryRun;
            this.setLiveActivity({
                kind: requireApproval ? 'approval' : 'hook',
            ...
            const between = await runBetweenIterations(this.buildBetweenIterationsOpts(betweenHook, iter, {
                goal: this.goal,
                template: this.template.name,
        ...
        const shouldReflect = this.template.reflection ||
        ...
        this.commandBoard?.appendBullet('Key Decisions', `[EXIT] Iter ${iter}: ${exitEval.shouldExit ? 'MET — completing loop' : 'continue'} — ${exitEval.reason}`);
        this.blackboard.post({
            type: 'result',
        ...
        this.emitState();
        console.error(`[Loop][engine] exit evaluation iter=${iter} shouldExit=${exitEval.shouldExit} reason="${exitEval.reason}"`);
        return { phasesCompleted, exitMet: exitEval.shouldExit };
    }
    /** Execute a single phase by config. */
    async runPhase(phaseConfig, ctx) {
        ...
        console.error(`[Loop][engine] phase transition → ${phase} iteration=${ctx.iteration} retryCount=${this.store.get().retryCount}`);
        ...
        this.emitState();
        this.hooks.onPhaseComplete?.(phase, result);
        console.error(`[Loop][engine] phase complete ${phase} success=${result.success} ` +
    ...
    setLiveActivity(activity) {
        this.store.setLiveActivity(activity
            ? {
                ...activity,
...
export class LoopEngineCoordinator {
    ...
    async onWaveComplete(waveNumber, hadBlockers) {
        ...
        const shouldRetry = critiqueResult?.shouldRetry ||
