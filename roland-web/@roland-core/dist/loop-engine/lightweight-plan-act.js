...
export async function runLightweightPlan(iteration, opts, extras = {}) {
    ...
    opts.commandBoard?.appendBullet('Key Decisions', `[Pure ClosedLoop] Plan iter ${iteration} — ${planDispatch.displayLabel} (${planDispatch.method})`);
    console.error(`[Loop][pure] Plan iteration=${iteration} path=lightweight dispatch=${planDispatch.method} model=${planDispatch.displayLabel}`);
    return {
...
export async function runLightweightAct(iteration, opts, extras = {}) {
    const router = opts.modelRouter ?? ModelRouter.fromConfig();
    ...
    console.error(`[Loop][pure] Act iteration=${iteration} path=lightweight dispatch=${actDispatch.method} model=${actDispatch.displayLabel}`);
    return {
        success: dispatch.success,
 ...
 */
//# sourceMappingURL=lightweight-plan-act.js.map