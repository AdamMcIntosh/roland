...
export class CritiqueEngine {
    ...
    critique(input) {
        ...
        const escalationThreshold = input.escalationThreshold ?? DEFAULT_ESCALATION_THRESHOLD;
        const enriched = { ...input, maxRetries, escalationThreshold };
...
function selectCritiqueLane(input, issues) {
    ...
    if (allCodeSpecific && issues.length <= 3)
        return 'coding';
    return 'critic';
}
function buildSummary(input, decision, reason) {
    if (decision === 'proceed') {
...
//# sourceMappingURL=critique-engine.js.map