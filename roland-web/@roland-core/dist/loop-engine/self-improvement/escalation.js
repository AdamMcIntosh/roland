 ...
 */
export function shouldEscalateToHuman(ctx) {
    if (ctx.maxRetries <= 0)
        return false;
    ...
    if (ctx.retryCount >= ctx.maxRetries) {
        console.error(`[Loop][escalation] retry budget exhausted retryCount=${ctx.retryCount} maxRetries=${ctx.maxRetries}`);
        return true;
    }
    ...
    if (ctx.consecutiveVerifyFailures >= threshold &&
        ctx.retryCount > 0 &&
        ctx.retryCount < ctx.maxRetries - 1) {
        console.error(`[Loop][escalation] consecutive verify failures threshold met ` +
...
export function escalationRetryDecision(ctx) {
