 ...
 */
import { DEFAULT_ESCALATION_THRESHOLD, escalationRetryDecision, shouldEscalateToHuman, } from './escalation.js';
/** Simple retry — re-run the full loop iteration when any gate fails. */
export function simpleRetryStrategy(input) {
 ...
 */
export function focusedRetryStrategy(input) {
    const escalationCtx = buildEscalationContext(input);
    if (shouldEscalateToHuman(escalationCtx)) {
        return {
            decision: 'escalate',
...
function countConsecutiveVerifyFailures(input) {
    let count = 0;
    ...
    if (count === 0 && input.verification && !input.verification.pass)
        count = 1;
    return count;
}
