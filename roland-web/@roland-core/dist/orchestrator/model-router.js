...
export class ModelRouter {
     ...
     */
    static checkOllamaHealth(baseUrl) {
        // Return cached result if fresh
        if (_ollamaHealthCache &&
     ...
     */
    static routeByComplexity(query) {
        const analysis = ComplexityClassifier.getDetailedAnalysis(query);
        ...
        if (complexity === 'local') {
            ...
            if (!config?.ollama?.enabled) {
                complexity = (config?.ollama?.fallback_to ?? 'simple');
            }
        }
        const context = {
     ...
     */
    static selectModelWithFallback(context) {
        const config = getConfig();
        if (!config) {
        ...
        const provider = isLocalTier ? 'local' : 'openrouter';
     ...
     */
    static getModelsForComplexity(complexity) {
        const config = getConfig();
        if (!config)
            return [];
     ...
     */
    static estimateCostPer1k(model) {
    ...
    static estimateCost(model, inputTokens, outputTokens) {
        const pricing = MODEL_PRICING[model];
        if (!pricing)
            return 0; // Unknown model — assume IDE subscription
     ...
     */
    static generateCostReport(model, inputTokens, outputTokens) {
