...
export function isMinimalGoal(goal) {
    const g = goal.toLowerCase().trim();
    if (requestsProductionHardening(goal))
        return false;
    const featureWork = /\b(implement|build|create|wire|extend)\b.*\b(api|endpoint|service|migration|middleware|repository|handler)\b/.test(g) ||
...
export function isFocusedFeatureGoal(goal) {
    const g = goal.toLowerCase();
    return (/\b(add|implement|wire|create)\b.*\b(middleware|endpoint|route|handler|service|module|plugin|adapter)\b/.test(g) ||
...
export function isScaffoldGoal(goal) {
    const g = goal.toLowerCase();
    if (requestsProductionHardening(goal) || isFocusedFeatureGoal(goal))
        return false;
    return (/\bscaffold\b.*\b(basic|minimal|simple|empty|stub|new)\b/.test(g) ||
 ...
 */
export function isGreenfieldGoal(goal) {
    const g = goal.toLowerCase();
    if (isScaffoldGoal(goal))
        return true;
    return (/\b(create|bootstrap|init|setup|scaffold)\b.*\b(project|app|repo|directory|folder|workspace)\b/.test(g) ||
...
export const HARDENING_BLOCKER_PATTERNS = [
    /\bstructured log/i,
    /\bilogg(?:er)?\b/i,
    /\brate limit/i,
...
//# sourceMappingURL=goal-scope.js.map