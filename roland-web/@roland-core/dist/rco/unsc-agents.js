 ...
 */
export function resolveUnscAgentsDir(referenceUrl) {
    try {
...
export function loadUnscAgents(agentsDir) {
    ...
    for (const f of fs.readdirSync(dir).filter((x) => x.endsWith('.yaml') || x.endsWith('.yml'))) {
        ...
        if (!parsed.success)
            continue;
        map.set(parsed.data.name ?? f.replace(/\.ya?ml$/, ''), parsed.data);
    }
    return map;
}
/** Build SDK `agents` map keyed by callsign slug (lowercase). */
export function toSdkAgentDefinitions(unscAgents) {
    const out = {};
...
export function legacyAgentToCallsign(agentName) {
...
//# sourceMappingURL=unsc-agents.js.map