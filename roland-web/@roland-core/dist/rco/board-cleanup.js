...
function shouldArchiveBlackboardEntry(entry, goalTokens) {
    ...
    if (entry.type === 'blocker' && entry.status === 'done')
        return true;
...
function shouldRemoveActiveTaskBullet(bullet, goalTokens) {
    const b = bullet.trim();
    if (!b || b.startsWith('_('))
        return true;
    if (STALE_TASK_STATUS_RE.test(b))
        return true;
    if (STALE_PENDING_RE.test(b)) {
        return !isGoalRelevant(b, goalTokens);
    }
    if (/\[blocked\]/i.test(b) && /\bcleared\b/i.test(b))
        return true;
    if (/\[blocked\]/i.test(b) && !isGoalRelevant(b, goalTokens))
        return true;
    if (/\[legacy blocker\]/i.test(b))
        return true;
    return false;
}
function shouldArchiveObjective(bullet, goalTokens) {
    const b = bullet.trim();
    if (!b || b.startsWith('_('))
        return false;
    if (/\[(complete|cancelled|archived)\]/i.test(b))
        return true;
    if (/\[(P[1-4]\s+)?active\]/i.test(b) && !isGoalRelevant(b, goalTokens))
        return true;
    return false;
}
function shouldRemoveIntel(bullet, goalTokens) {
    const b = bullet.trim();
    if (!b || b.startsWith('_('))
        return false;
    if (STALE_INTEL_RE.test(b))
        return true;
    if (/\[BLOCKER cleared\]/i.test(b))
        return true;
    if (/\[ESCALATION\]/i.test(b) && !isGoalRelevant(b, goalTokens))
        return true;
    return false;
}
/** Archive stale blackboard.json entries from prior missions. */
export function cleanupMachineBlackboard(blackboard, options = {}) {
    const goalTokens = tokenize(options.goal ?? '');
    const titles = [];
    let archived = 0;
    for (const entry of blackboard.read()) {
        if (!shouldArchiveBlackboardEntry(entry, goalTokens))
            continue;
        titles.push(entry.title);
        if (!options.dryRun)
            blackboard.archive(entry.id);
        archived++;
    }
    return { archived, titles };
}
/** Clean command-blackboard.md — remove stale tasks, archive old objectives, reset agents. */
export function cleanupCommandBlackboard(board, options = {}) {
    const goalTokens = tokenize(options.goal ?? '');
    const sections = board.readSections();
    const activeTasksRemoved = [];
    const objectivesArchived = [];
    const intelRemoved = [];
    const keptTasks = [];
...
export function cleanupBoardsForNewMission(stateDir, goal, options = {}) {
...
export function formatCleanupReport(result) {
    ...
    if (cb.activeTasksRemoved.length) {
        ...
        for (const t of cb.activeTasksRemoved.slice(0, 5))
            lines.push(`  · ${t.slice(0, 80)}`);
    }
    if (cb.objectivesArchived.length) {
        lines.push(`Mission Objectives archived: ${cb.objectivesArchived.length}`);
        for (const t of cb.objectivesArchived.slice(0, 3))
            lines.push(`  · ${t.slice(0, 80)}`);
    }
    ...
    if (cb.agentsReset)
        lines.push('Agent Status: reset to idle');
    if (result.blackboardArchived === 0 &&
...
//# sourceMappingURL=board-cleanup.js.map