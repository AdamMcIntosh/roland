...
export function resetLoopArtifactsForNewMission(stateDir, log = noopLog) {
    let reset = false;
...
export function resetHitlStateForNewMission(stateDir, log = noopLog) {
    try {
 ...
 */
export function prepareMissionStart(stateDir, goal, options = {}, log = noopLog) {
    ...
    const result = cleanupPreviousRuns(stateDir, goal, {
...
function appendMissionArchiveLine(stateDir, record) {
...
export function readMissionMetaFile(stateDir) {
    const meta = readJsonFile(path.join(stateDir, MISSION_META_FILE), null);
    return meta && typeof meta === 'object' ? meta : null;
}
export function writeMissionMetaFile(stateDir, meta) {
...
export function readSupervisorRecord(stateDir) {
    const rec = readJsonFile(path.join(stateDir, SUPERVISOR_PID_FILE), null);
    return rec?.pid ? rec : null;
}
export function readRunStateRecord(stateDir) {
    const rs = readJsonFile(path.join(stateDir, RUN_STATE_FILE), null);
    return rs?.runId ? rs : null;
}
export function isSupervisorAlive(stateDir) {
    const rec = readSupervisorRecord(stateDir);
    return Boolean(rec?.pid && isProcessAlive(rec.pid));
}
export function isRunStateActive(stateDir, now = Date.now()) {
    const rs = readRunStateRecord(stateDir);
    if (!rs?.runId || !rs.status)
        return false;
...
export function isMissionMetaActive(meta, stateDir) {
    if (!meta || meta.status === 'archived' || meta.status === 'completed')
        return false;
    if (isSupervisorAlive(stateDir))
        return true;
    return isRunStateActive(stateDir);
}
/** Return mission-meta only when it represents a live mission in this state dir. */
export function readActiveMissionMeta(stateDir) {
    const meta = readMissionMetaFile(stateDir);
    return isMissionMetaActive(meta, stateDir) ? meta : null;
}
export function archiveMissionMeta(stateDir, reason, log = noopLog) {
    const meta = readMissionMetaFile(stateDir);
    if (!meta || meta.status === 'archived' || meta.status === 'completed')
        return false;
    const archived = {
 ...
 */
export function sanitizeStaleMissionState(stateDir, log = noopLog, now = Date.now()) {
    const actions = [];
    let changed = false;
    ...
    const runActive = Boolean(runState?.runId
        && runState.status
 ...
 */
export function isolateProjectMissionState(stateDir, log = noopLog) {
 ...
 */
export function cleanupPreviousRuns(stateDir, goal, options = {}, log = noopLog) {
 ...
 */
export function readActiveRunStateForClient(stateDir, now = Date.now()) {
    const rs = readRunStateRecord(stateDir);
    if (!rs?.runId)
        return null;
    if (isSupervisorAlive(stateDir))
        return rs;
    if (isRunStateActive(stateDir, now))
        return rs;
    return null;
}
function sleepMs(ms) {
 ...
 */
export async function waitForSupervisorReady(stateDir, options = {}) {
...
function resolveSupervisorLogFile(stateDir) {
    const rec = readSupervisorRecord(stateDir);
...
function tailSupervisorLog(logFile, lines = 40) {
...
export function buildSupervisorStartDiagnostics(stateDir, context) {
