...
export class Notifier {
    ...
    async notify(payload) {
        if (!this.shouldFire(payload.event))
            return;
    ...
    async desktopNotify(title, body) {
        try {
            ...
            const { default: n } = await import('node-notifier');
        ...
        try {
            ...
            const esc = (s) => s.replace(/['"\\]/g, ' ').slice(0, 100);
            if (p === 'win32') {
                execSync(`powershell -NonInteractive -Command "` +
                    ...
                    `[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('Roland').Show([Windows.UI.Notifications.ToastNotification]::new($template))"`, { timeout: 5000, stdio: 'ignore' });
            }
            else if (p === 'darwin') {
                execSync(`osascript -e 'display notification "${esc(body)}" with title "${esc(title)}"'`, { timeout: 5000, stdio: 'ignore' });
            }
            else {
    ...
    async webhookNotify(title, body, payload) {
    ...
    buildBody(p) {
        ...
        switch (p.event) {
            case 'complete': {
                lines.push(`"${goal}"`);
                const parts = [];
                if (p.tasksCompleted !== undefined)
                    parts.push(`${p.tasksCompleted} task${p.tasksCompleted !== 1 ? 's' : ''}`);
                if (p.wavesRun !== undefined)
                    parts.push(`${p.wavesRun} wave${p.wavesRun !== 1 ? 's' : ''}`);
                if (p.durationMs !== undefined)
                    parts.push(formatDurationShort(p.durationMs));
                if (parts.length)
                    lines.push(parts.join(' · '));
                const b = p.blockersEncountered ?? 0;
                if (b > 0)
                    lines.push(`⚠️  ${b} blocker${b !== 1 ? 's' : ''} were encountered and resolved`);
                break;
            }
            case 'error': {
                lines.push(`"${goal}"`);
            ...
            case 'blocker': {
                lines.push(`"${goal}"`);
                if (p.waveNumber)
                    lines.push(`Wave ${p.waveNumber}`);
                if (p.blockerDescription)
                    lines.push(`Blocked on: ${p.blockerDescription.slice(0, 120)}`);
                else if (p.summary)
        ...
        if (p.contextLine)
            lines.push(p.contextLine);
        return lines.join('\n');
    }
    ntfyPriority(event) {
        if (event === 'error' || event === 'blocker')
            return 'high';
        if (event === 'hitl-pause')
            return 'urgent';
        return 'default';
...
// ── Config loader ─────────────────────────────────────────────────────────────
export function parseNotifierConfig(raw) {
