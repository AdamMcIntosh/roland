...
export function stripMissionCompleteFooter(synthesis) {
 ...
 */
export function extractNextStepsSection(synthesis) {
    const match = synthesis.match(NEXT_STEPS_SECTION);
    if (!match)
        return { body: synthesis.trimEnd(), nextSteps: null };
...
function stripNonDisplaySections(body, minimal) {
    const alwaysStrip = [
        'Memory Extract',
        'Knowledge Update',
...
function collapseEmptyActionTiers(body) {
    let out = body;
...
export function sanitizeReleaseBlockersForMinimalGoal(body) {
...
export function compactSynthesisBody(body, goal) {
    const minimal = isMinimalGoal(goal);
    let out = stripNonDisplaySections(body, minimal);
    if (minimal)
        out = sanitizeReleaseBlockersForMinimalGoal(out);
    return out.trimEnd();
}
function buildDefaultNextSteps(ctx) {
...
export function formatMissionCompleteFooter(ctx, nextSteps) {
    ...
    const statusLine = ctx.blockersEncountered > 0
...
export function ensureFooterIsTerminal(output) {
 ...
 */
export function finalizeSynthesisOutput(synthesis, ctx) {
