...
import os from 'os';
function suggestedAgents(state) {
...
function buildDynamicHints(state) {
    const blob = `${state.task} ${Object.values(state.outputs).join(' ')}`.toLowerCase();
    const hints = ['## Dynamic hints'];
    if (/bug|fix|regression|broken/.test(blob))
        hints.push('- Consider the BugFix recipe for follow-up work.');
    if (/security|audit|vuln/.test(blob))
        hints.push('- Consider SecurityAudit for deeper review.');
    if (/refactor|cleanup|debt/.test(blob))
        hints.push('- Consider Refactor for structural cleanup.');
    return hints.length > 1 ? `${hints.join('\n')}\n` : '';
}
export function exportCursor(opts) {
...
//# sourceMappingURL=exportCursor.js.map