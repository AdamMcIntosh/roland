...
import { GitCommitApprovalQueue, GIT_COMMIT_APPROVAL_FILE, } from '../loop-engine/git-commit-approval.js';
function isTTY() {
    return Boolean(process.stderr.isTTY);
}
function formatExpiry(request) {
...
function truncatePreview(preview, maxLines = 6) {
...
export function readPendingGitCommitApproval(stateDir) {
...
export function resolveGitCommitApprovalId(stateDir, explicitId) {
    const queue = new GitCommitApprovalQueue(stateDir);
    const current = queue.read();
    if (!current) {
...
export function printPendingGitCommitApproval(stateDir) {
...
export function decideGitCommitApprovalCli(decision, opts = {}) {
    const stateDir = opts.stateDir ?? '.roland';
    const explicitId = opts.id?.trim() || undefined;
    const hasDecisionPayload = (decision === 'approve' && Boolean(opts.message?.trim())) ||
    ...
    if (Date.now() >= request.timeoutAt) {
        process.stderr.write(`⚠️  Approval window expired at ${new Date(request.timeoutAt).toLocaleString()} — attempting decision anyway.\n`);
    }
    const queue = new GitCommitApprovalQueue(stateDir);
    ...
    const approvedMessage = decision === 'approve'
...
function parseCliArgs(argv) {
    ...
    const positional = filtered.filter((_, i) => i !== messageIdx &&
        i !== messageIdx + 1 &&
        i !== reasonIdx &&
        i !== reasonIdx + 1);
...
function printDecisionFeedback(result) {
    if (!result.ok) {
...
export function runApproveCommitCli(argv) {
...
export function runRejectCommitCli(argv) {
...
export function printGitCommitApprovalStatus(stateDir) {
    const pending = readPendingGitCommitApproval(stateDir);
    const dim = (s) => `\x1b[2m${s}\x1b[0m`;
    const y = (s) => `\x1b[33m${s}\x1b[0m`;
...
//# sourceMappingURL=git-commit-approval-cli.js.map