...
import { RcoConfigSchema, RcoRecipeSchema, AgentYamlSchema } from './types.js';
 ...
 */
export function resolveAgentsDir(referenceUrl, override) {
    if (override)
        return override;
    try {
...
export function loadRcoConfig(configPath = 'config.yaml') {
...
export function loadAgentYaml(filePath) {
    const content = fs.readFileSync(filePath, 'utf-8');
    const doc = yaml.load(content);
    const parsed = AgentYamlSchema.safeParse(doc);
    if (!parsed.success)
        throw new Error(`Invalid agent YAML ${filePath}: ${parsed.error.message}`);
    return parsed.data;
}
export function loadAllAgents(agentsDir = DEFAULT_AGENTS_DIR, opts = {}) {
    const map = new Map();
    ...
    for (const f of files) {
        ...
        if (opts.excludeVariants && VARIANT_SUFFIX_RE.test(baseName))
            continue;
        try {
...
export function loadRecipe(recipeName, recipesDir = DEFAULT_RECIPES_DIR) {
