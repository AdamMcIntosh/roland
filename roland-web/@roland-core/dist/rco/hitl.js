...
export class HitlQueue {
    ...
    isPaused() {
        try {
            const s = JSON.parse(fs.readFileSync(this.stateFile, 'utf-8'));
    ...
    readQueue() {
        try {
            return JSON.parse(fs.readFileSync(this.cmdFile, 'utf-8'));
        }
        catch {
            return [];
        }
     ...
     */
    _updateObserverState(cmd) {
        const queue = this.readQueue();
        let existing = { paused: false, updatedAt: 0 };
        try {
...
export function writeHitlCommand(stateDir, cmd) {
    const q = new HitlQueue(stateDir);
    q.push(cmd);
}
/** Print status of HITL state to stderr. */
export function printHitlStatus(stateDir) {
    const q = new HitlQueue(stateDir);
    ...
    let queueLen = 0;
    try {
        const arr = JSON.parse(fs.readFileSync(cmdFile, 'utf-8'));
        queueLen = arr.length;
    }
    catch { /* no file */ }
