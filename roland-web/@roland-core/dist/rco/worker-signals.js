 ...
 */
export function parseWorkerSignals(output) {
    const blockers = [];
    const messages = [];
    ...
    const inlineRe = /(?:\*\*BLOCKED?:\*\*|(?:⚠️|🚨)\s+BLOCKED?:|^BLOCKING ISSUE:)\s*(.+)/gim;
    let m;
 ...
 *   BLOCKING ISSUE: …
 */
export function hasBlockerSignal(output) {
    return (/^##\s*(?:🚨\s*)?BLOCKER\b/im.test(output) ||
        /\*\*BLOCKED?:\*\*/i.test(output) ||
        /(?:⚠️|🚨)\s+BLOCKED?:/i.test(output) ||
        /^BLOCKING ISSUE:/im.test(output));
}
//# sourceMappingURL=worker-signals.js.map