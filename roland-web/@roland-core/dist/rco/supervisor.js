...
export function readSupervisorRecord(stateDir) {
    try {
        return JSON.parse(fs.readFileSync(path.join(stateDir, SUPERVISOR_PID_FILE), 'utf-8'));
    }
    catch {
        return null;
    }
}
function writeSupervisorRecord(stateDir, record) {
    fs.mkdirSync(stateDir, { recursive: true });
    fs.writeFileSync(path.join(stateDir, SUPERVISOR_PID_FILE), JSON.stringify(record, null, 2), 'utf-8');
}
function removeSupervisorRecord(stateDir) {
    try {
        fs.rmSync(path.join(stateDir, SUPERVISOR_PID_FILE), { force: true });
    }
    catch { /* ignore */ }
}
// ── Formatting helpers ────────────────────────────────────────────────────────
...
function phaseLabel(status, wave) {
    switch (status) {
...
export function bgStatus(stateDir, json = false) {
    ...
    try {
        ...
        const hs = JSON.parse(fs.readFileSync(hitlStateFile, 'utf-8'));
        if (hs.paused)
            hitlPaused = true;
        if (hs.abortPending)
            abortPending = true;
    ...
    const row = (label, value) => w(`  ${dim(label.padEnd(12))}  ${value}`);
...
function resolveLogFile(stateDir) {
    const rec = readSupervisorRecord(stateDir);
...
export function bgLogs(stateDir, lines = 50) {
    ...
    try {
        ...
        process.stderr.write('Could not read log file: ' + e.message + '\n');
    }
}
/**
 ...
 */
export function bgLogsFollow(stateDir) {
    const logFile = resolveLogFile(stateDir);
    if (!logFile) {
        process.stderr.write('No background logs found.\n');
        return;
 ...
 */
export function bgStop(stateDir) {
    const rec = readSupervisorRecord(stateDir);
    if (!rec) {
        process.stderr.write('No background run found.\n');
        return;
    ...
    try {
        ...
        let existing = [];
        try {
            existing = JSON.parse(fs.readFileSync(hitlPath, 'utf-8'));
        }
        catch { /* new */ }
 ...
 */
async function supervisorWorkerMain(argv) {
    ...
    if (lastError)
        process.stderr.write(`[Supervisor] Last error: ${lastError.message}\n`);
    removeSupervisorRecord(stateDir);
    process.exit(1);
}
// ── Standalone entry point ────────────────────────────────────────────────────
const _thisFile = fileURLToPath(import.meta.url);
const isDirectRun = process.argv[1] === _thisFile ||
    process.argv[1]?.replace(/\.ts$/, '.js') === _thisFile;
if (isDirectRun && process.argv[2] === '--background-worker') {
    supervisorWorkerMain(process.argv.slice(2)).catch((e) => {
