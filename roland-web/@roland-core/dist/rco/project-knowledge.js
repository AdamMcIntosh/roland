 ...
 */
export function loadProjectKnowledge(projectRoot = process.cwd()) {
    ...
    const files = found.map(({ def, content, filepath }) => {
        const budget = Math.max(MIN_FILE_CHARS, Math.floor((def.weight / totalWeight) * MAX_KNOWLEDGE_CHARS));
    ...
    for (const f of files) {
        ...
        if (snippet.length > allocated) {
            ...
            if (lastNl > allocated * 0.75)
                snippet = snippet.slice(0, lastNl);
    ...
    const truncatedNote = truncated
        ? '\n\n> **Context budget note:** Some files were trimmed. ' +
            ...
            files
                ...
                .join(', ') + '.'
        : '';
    const injectionBlock = `## Project Knowledge\n\n` +
        ...
        sections.join('\n\n---\n\n') +
        truncatedNote;
    const summary = `${files.length} file(s) loaded: ${files.map((f) => f.filename).join(', ')}` +
 ...
 */
export function parseKnowledgeUpdate(synthesis) {
    const blockMatch = synthesis.match(/##\s+Knowledge Update\s*\n([\s\S]*?)(?=\n##\s|\s*$)/i);
    if (!blockMatch)
        return [];
    const block = blockMatch[1];
    // Preferred: **DECISIONS.md:** sub-section
    const decisionsMatch = block.match(/\*\*DECISIONS\.md:\*\*\s*\n([\s\S]*?)(?=\*\*[A-Z]|\s*$)/i);
 ...
 */
export function appendDecisions(synthesis, goal, runId, projectRoot = process.cwd()) {
