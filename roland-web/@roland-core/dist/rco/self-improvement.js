 ...
 */
export async function collectHumanFeedback(goal, opts) {
    if (!opts.isTTY)
        return null;
    ...
    return new Promise((resolve) => {
        ...
        const onLine = (input) => {
            ...
            if (!m) {
                settle(null, '(skipped — expected a number 1–10)');
                return;
            }
            const rating = parseInt(m[1], 10);
            if (rating < 1 || rating > 10) {
                settle(null, '(skipped — rating must be 1–10)');
                return;
            }
            const notes = m[2]?.trim() || undefined;
            settle({ rating, notes }, `Recorded ${rating}/10${notes ? ` — "${notes.slice(0, 60)}"` : ''}`);
        };
        const timer = setTimeout(() => settle(null, '(auto-skipped)'), opts.timeoutSeconds * 1_000);
        if (opts.rl) {
 ...
 */
export function parsePlanCitations(planText) {
    const m = planText.match(/##\s+Memory Citations?\s*\n([\s\S]*?)(?=\n##\s|\s*```|\s*$)/i);
    if (!m)
        return [];
    const block = m[1];
    if (/no prior memory/i.test(block))
        return [];
    return block
        .split('\n')
        .map((l) => l.trim())
 ...
 */
export function parseSelfCritique(retroText) {
    const m = retroText.match(/##\s+Planning Self-Critique\s*\n([\s\S]*?)(?=\n##\s|\s*$)/i);
    if (!m)
        return null;
    const text = m[1].trim();
 ...
 */
export function buildRetrospectivePrompt(goal, synthesis, taskSummary, currentMemory, feedback) {
 ...
 */
export function parseRetrospectiveOutput(text) {
    const blockMatch = text.match(/##\s+Retrospective Memory Update\s*\n([\s\S]*?)(?=\n##\s|\s*$)/i);
    if (!blockMatch)
        return null;
    const block = blockMatch[1];
    if (/nothing new to document/i.test(block))
        return null;
    const SECTION_MAP = {
    ...
    const result = Object.fromEntries(MEMORY_SECTIONS.map((s) => [s, []]));
    const blockRe = /\*\*([^:*\n]+):\*\*\s*\n([\s\S]*?)(?=\*\*[^:*\n]+:\*\*|$)/g;
    let m;
    let totalBullets = 0;
 ...
 */
export async function showMemoryProposal(proposed, existing, opts) {
    // Compute diff — only bullets not already in memory (strip [×N] before compare)
    const diff = {};
    let totalNew = 0;
    for (const section of MEMORY_SECTIONS) {
        const existingKeys = new Set((existing[section] ?? []).map((b) => b.toLowerCase().replace(/^\[×\d+\]\s*/, '').slice(0, 60)));
        const fresh = (proposed[section] ?? []).filter((b) => !existingKeys.has(b.toLowerCase().replace(/^\[×\d+\]\s*/, '').slice(0, 60)));
        if (fresh.length > 0) {
            diff[section] = fresh;
    ...
    if (totalNew === 0)
        return 'accepted'; // nothing genuinely new
    if (!opts.isTTY || opts.quiet)
        return 'accepted'; // non-interactive — auto-accept silently
    // ── Render the proposal ──────────────────────────────────────────────────
    const cols = Math.min((process.stderr.columns ?? 80), 90);
    const hr = '─'.repeat(cols - 4);
    const bold = (s) => `\x1b[1m${s}\x1b[0m`;
    const dim = (s) => `\x1b[2m${s}\x1b[0m`;
    const green = (s) => `\x1b[32m${s}\x1b[0m`;
    const cyan = (s) => `\x1b[36m${s}\x1b[0m`;
    const mg = (s) => `\x1b[35m${s}\x1b[0m`;
    const w = process.stderr.write.bind(process.stderr);
    w('\n');
 ...
 */
export function applyRetroUpdate(incoming, stateDir, goal, runId) {
