...
function extractJsonBlock(text) {
    ...
    try {
        ...
        console.error('[Team] Failed to parse PM JSON block:', err.message);
        console.error('[Team] Raw block (first 500 chars):', match[1].slice(0, 500));
        return null;
    }
}
function isTeamPlan(v) {
    return typeof v === 'object' && v !== null && 'tasks' in v && Array.isArray(v.tasks);
}
...
function buildMinimalSynthesis(goal, taskResults) {
    const lines = [
...
async function runConcurrent(factories, limit) {
    ...
    for (let i = 0; i < slots; i++) {
        if (i > 0 && AGENT_WARMUP_DELAY_MS > 0) {
            await new Promise((r) => setTimeout(r, AGENT_WARMUP_DELAY_MS));
        }
...
async function callCursorAgent(agentName, modelId, prompt, circuitBreaker) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            if (netError) {
                console.error(`[Team]   🌐 ${agentName} — connection error, retrying in ${(delay / 1000).toFixed(1)}s` +
                    ` (attempt ${attempt}/${maxAttempts}) — run is still alive; use 'roland status' to monitor`);
            }
            else {
                console.error(`[Team]   ⚠️  ${agentName} attempt ${attempt} failed: ${lastErr.message.slice(0, 100)}` +
    ...
    console.error(`[Team]   💀 ${agentName} failed after ${maxAttempts} attempts` +
...
export async function runTeam(opts) {
    ...
    async function executeTask(task) {
        ...
        const agentYaml = rosterMap.get(agentKey) ?? { name: task.agent };
    ...
    while (remaining.length > 0) {
        ...
        const waveResults = await runConcurrent(ready.map((task) => () => executeTask(task)), MAX_CONCURRENT_AGENTS);
    ...
    const synthesisFailed = (s) => !s.trim() || s.trim().length < 200 || s.includes('## 🚨 BLOCKER');
    ...
    if (!noImprove) {
        ...
        let humanFeedback;
        if (interactive && Boolean(process.stderr.isTTY)) {
        ...
        if (critique) {
            ...
            for (const line of lines)
                console.error(`[Team]   ${line.slice(0, 115)}`);
        }
    }
    // ── Save usage record ─────────────────────────────────────────────────────
    ...
    console.error(`[Team] Usage: ~${runUsage.totalTokens.toLocaleString()} est. tokens` +
