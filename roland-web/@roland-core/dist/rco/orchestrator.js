...
function resolveWorkerPath(referenceUrl = import.meta.url) {
...
function adaptiveStepCount(task) {
...
function loadOrchestratorRecipe(recipeName, recipesDir, task, executionMode) {
...
function resolveAgentYaml(stepAgent, agents, recipe) {
    const key = stepAgent.toLowerCase();
    const fromDir = agents.get(key);
    if (fromDir)
        return { ...fromDir, name: stepAgent };
...
function buildStepInput(step, task, state, previousAgent) {
    if (step.input)
        return step.input.replace(/\{\{user_task\}\}/g, task);
    if (previousAgent && state.outputs[previousAgent] !== undefined) {
        return String(state.outputs[previousAgent]);
    }
    return task;
}
function shouldLoopBack(step, output) {
    if (!step.loop_if)
        return false;
    return output.toLowerCase().includes(step.loop_if.toLowerCase());
}
function synthesizeOutput(state) {
    const sections = [];
...
function createInitialState(recipeName, task) {
    const now = Date.now();
    return {
...
function defaultRunWorker(workerPath, input) {
...
async function invokeWorker(runWorker, workerPath, input, workerRetries, workerTimeoutMs) {
    let lastError;
        ...
        try {
            const result = await Promise.race([
                ...
                new Promise((_, reject) => {
                    setTimeout(() => reject(new Error(`Worker timeout after ${workerTimeoutMs}ms`)), workerTimeoutMs);
                }),
            ]);
            if (result.success)
                return result;
...
function persistState(stateFilePath, state) {
...
export async function runOrchestrator(opts) {
    const { recipeName, task, configPath, agentsDir, recipesDir, stateFilePath = path.join(process.cwd(), '.rco-state.json'), runWorker = defaultRunWorker, workerRetries = DEFAULT_WORKER_RETRIES, workerTimeoutMs = DEFAULT_WORKER_TIMEOUT_MS, executionMode, } = opts;
    ...
    let state = readStateUnlocked(stateFilePath) ?? createInitialState(recipe.name, task);
