 ...
 */
import { ClosedLoop, LoopTemplates, readLoopPmSession, resolvePmIntegrationStatus } from '../loop-engine/index.js';
...
export function hasLoopTemplate(loopTemplate) {
 ...
 * ```
 */
export async function runClosedLoopMission(opts) {
    const { goal, stateDir = '.roland', loopTemplate, onLoopStateChange, onPlanReady, onSynthesizing, loopRunner, } = opts;
    if (!hasLoopTemplate(loopTemplate)) {
    ...
    for (const role of ['pm', 'coding', 'critic', 'verifier']) {
    ...
    commandBoard.appendBullet('Mission Objectives', `Loop template: ${templateName} · ${pmStatus.enabled ? 'PM-Enhanced (Legacy)' : 'Pure ClosedLoop'}`);
    ...
    const pmNote = pmSession
        ...
        : '';
    console.error(`[Loop] Usage: ~${runUsage.totalTokens.toLocaleString()} est. tokens` +
        ` | ~$${runUsage.totalCostUsd.toFixed(4)} est. cost` +
        ` | ClosedLoop harness${pmNote}` +
        ` | saved to ${stateDir}/usage-history.json`);
    console.error(`[Loop] Mission complete status=${result.status} iterations=${result.iterationsRun} ` +
...
function buildClosedLoopSynthesis(goal, result, stateDir) {
    const lines = [
    ...
    if (result.state.lastVerification) {
        lines.push('## Verification', '', result.state.lastVerification.summary, result.state.lastVerification.confidence !== undefined
            ? `Confidence: ${result.state.lastVerification.confidence}`
            : '', '');
    }
    lines.push('## Loop Engineering', '', `- Specialist spawn intents: ${result.spawnCount}`, `- Phases completed: ${result.phasesCompleted}`, `- Loop directory: ${result.loopDir}`, `- Model routing: ${ModelRouter.fromConfig().formatRoutingSummary()}`);
    const pmSession = readLoopPmSession(stateDir);
    if (pmSession) {
        lines.push(`- [DEPRECATED] PM Team path: ${pmSession.executionPath} (${pmSession.routingReason})`, `- PM waves in Act: ${pmSession.wavesRun}`, `- PM blockers: ${pmSession.blockersEncountered}`);
    }
    lines.push('', '## Next Steps', '', '1. Review the PR draft above (also saved under `.roland/closed-loop-pr.json`).', '2. Inspect loop memory and reflections under `.roland/loops/`.', '3. Run `roland board-status --concise` for battlespace summary.', '');
    return lines.join('\n');
}
/**
