...
export function isComplexGoalForDag(goal) {
 ...
 */
export function isDagPlanningEnabled(goal, env = process.env) {
...
export function buildEdgesFromTasks(tasks) {
    const edges = [];
 ...
 */
export function computeCriticalPath(nodeIds, edges) {
    if (nodeIds.length === 0)
        return [];
    ...
    if (order.length !== nodeIds.length)
        return nodeIds.slice(0, Math.min(3, nodeIds.length));
    let end = nodeIds[0];
    let best = 0;
    for (const id of nodeIds) {
...
export function tasksToNodes(tasks, planningMode) {
    return tasks.map((t) => ({
        id: t.id,
        title: t.title,
        agent: t.agent,
...
export function getReadyNodeIds(nodes, completedIds) {
    return nodes
...
export function formatMissionGraphSummary(snapshot) {
...
export function formatNodeDagContext(snapshot, taskId) {
...
export class MissionDagStore {
    ...
    constructor(stateDir, initial) {
        ...
        if (initial) {
            ...
            this.snapshot = JSON.parse(fs.readFileSync(this.filePath, 'utf-8'));
        }
        else {
    ...
    refreshReadyStates(completedIds) {
        for (const node of this.snapshot.nodes) {
            if (node.status === 'done' || node.status === 'blocked' || node.status === 'in_progress')
                continue;
...
//# sourceMappingURL=mission-dag.js.map