...
import { buildConventionalPrTitle, buildPrDescription, buildTaskCommitMessage, cleanPrDescription, extractShortSummary, stripMissionNoise, } from './pr-format.js';
...
export function loadGitWorkflowConfig(stateDir) {
    const cfg = readJsonFile(path.join(stateDir, 'config.json'));
    const gitSection = (cfg.git ?? {});
    const ghSection = (cfg.github ?? {});
    const envEnabled = envTruthy('ROLAND_GIT_ENABLED');
    const envDraft = envTruthy('ROLAND_CREATE_DRAFT_PR');
    const token = (typeof ghSection.token === 'string' ? ghSection.token : undefined)
        ?? process.env.ROLAND_GITHUB_TOKEN
        ?? process.env.GITHUB_TOKEN
        ?? process.env.GH_TOKEN;
    const owner = (typeof ghSection.owner === 'string' ? ghSection.owner : undefined)
        ?? process.env.ROLAND_GITHUB_OWNER
        ?? process.env.GITHUB_OWNER;
    const repo = (typeof ghSection.repo === 'string' ? ghSection.repo : undefined)
...
export function isExecutorAgent(agent) {
...
export function buildTaskBranchName(taskId, title) {
    const slug = slugifyTitle(title);
    const short = shortTaskId(taskId);
    const name = `task-${short}-${slug}`;
    return name.slice(0, 120);
}
function statusLabelForPhase(phase, branch, prUrl) {
    switch (phase) {
        case 'branch_created':
...
function resolveGithubTarget(cwd, cfg) {
...
export function readTaskGitPayload(stateDir) {
    return readTaskGitStore(stateDir);
}
function readTaskGitStore(stateDir) {
...
function writeTaskGitStore(stateDir, store) {
    fs.mkdirSync(stateDir, { recursive: true });
    fs.writeFileSync(path.join(stateDir, TASK_GIT_FILE), JSON.stringify({ ...store, updatedAt: Date.now() }, null, 2), 'utf-8');
}
// ── Workflow class ────────────────────────────────────────────────────────────
export class TaskGitWorkflow {
    ...
    postBlackboard(task, info) {
        ...
        if (info.branch) {
            this.opts.commandBoard.appendBullet('Artifacts', `[${task.id}] branch \`${info.branch}\`${info.prUrl ? ` · PR ${info.prUrl}` : ''}`);
        }
    }
    ...
    async onTaskComplete(task, startInfo) {
    ...
    async createDraftPr(cwd, branch, title, body, target) {
        if (ghAvailable()) {
            try {
                const bodyFile = path.join(this.opts.stateDir, `.pr-body-${Date.now()}.md`);
                fs.writeFileSync(bodyFile, body, 'utf-8');
                try {
                    const out = execSync(`gh pr create --draft --title ${JSON.stringify(title)} --body-file ${JSON.stringify(bodyFile)} --head ${JSON.stringify(branch)}`, { cwd, encoding: 'utf-8', stdio: ['ignore', 'pipe', 'pipe'] }).trim();
                    ...
                    const view = execSync(`gh pr view --head ${JSON.stringify(branch)} --json url,number`, { cwd, encoding: 'utf-8', stdio: ['ignore', 'pipe', 'pipe'] });
                    const parsed = JSON.parse(view);
                    if (parsed.url)
                        return { url: parsed.url, number: parsed.number ?? 0 };
                }
                finally {
                    try {
...
//# sourceMappingURL=task-git-workflow.js.map