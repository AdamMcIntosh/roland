...
import { WorkerInputSchema, WorkerOutputSchema } from './types.js';
 ...
 */
async function getResponseViaCursorSDK(prompt, agentName, model) {
 ...
 */
async function getResponseViaCursorSDKWithRetry(prompt, agentName, model) {
    const maxAttempts = 5;
...
async function runAsync(input) {
    ...
    const prompt = buildClaudeToolCallingPrompt({
        ...
        taskContext,
        stepInput,
        stateSummary: state ? { currentStep: state.currentStep, loopCount: state.loopCount } : undefined,
    ...
    if (process.env.CURSOR_API_KEY) {
        ...
        try {
            ...
            log('cursor-sdk', `Failed: ${err.message}`);
            sendResult({ type: 'result', success: false, output: '', error: err.message });
    ...
    let dotGraph = responseParsed.dotGraph;
...
if (typeof process.send === 'function') {
    process.on('message', (msg) => run(msg));
}
else {
