...
import { ensureMissionProjectContext, resolveMcpProjectContext, } from '../utils/mcp-project-context.js';
...
function getSection(text, needle) {
...
function firstSentences(body, maxChars = 200) {
...
export async function runTeamCli(argv) {
    ...
    if (web) {
            ...
            result = await runTeam({
                ...
                onWaveStart: (waveNumber, tasks) => {
                    ...
                    for (const t of tasks)
                        webWaveEntries.set(t.id, { agent: t.agent, title: t.title, hadBlocker: false });
                    out('');
                ...
                onWaveComplete: (_w, decision) => {
                    ...
                    if (decision.decision !== 'continue') {
                        ...
                        for (const u of (decision.unblocks ?? []))
                            out(`  ↑ Unblock ${u.forAgent}: ${u.message}`);
                    }
                },
                ...
                onHitlPause: (p) => {
                    ...
                    if (!p)
                        runState.clearConnectionDropped();
                    if (p)
                        out('⏸  Paused — send `roland resume` to continue.');
                },
        ...
        if (summaryBody) {
            out(firstSentences(summaryBody));
            out('');
        }
        ...
        if (nextBody) {
            ...
            for (const line of nextBody.split('\n')) {
                ...
                if (!t)
                    continue;
                out(t.replace(/^(\d+\.\s+)(Run|Start|Deploy|Commit|Check|Open|Review|Execute|Install|Configure|Push|Test|Verify|Launch):\s+/i, '$1'));
            }
        }
        return;
    }
    ...
    const useTui = !noTui && Boolean(process.stdout.isTTY);
    if (useTui) {
        const runStart = Date.now();
        const runState = new RunStateWriter(stateDir, goal);
            ...
            cmdDispatch.fn = (cmd) => {
                ...
                switch (verb) {
                    case '/status': {
                        ...
                        const doneCount = state.tasks.filter((t) => t.status === 'done' || t.status === 'blocked').length;
                        const lines = [
                        ...
                        if (state.pmNotes)
                            lines.push(`PM: ${state.pmNotes.slice(0, 90)}`);
    ...
    const result = await runTeam({
        ...
        onWaveStart: (waveNumber, tasks) => {
            ...
            err(rule());
            if (parallel) {
                err(`  ${c.bold(`Wave ${waveNumber}`)}  ${c.dim('·')}  ${tasks.length} task${tasks.length !== 1 ? 's' : ''} in parallel  ` +
                    `${c.dim('[')}${bar}${c.dim(']')}  ${c.dim(rs.completedTasks + '/' + rs.totalTasks + ' tasks done')}`);
            }
            else {
                const task = tasks[0];
                err(`  ${c.bold(`Step ${waveNumber}`)}  ${c.dim('·')}  ${c.cyan(task?.agent ?? '?')}  ${c.dim('·')}  ` +
        ...
        onTaskComplete: (id, agent, output, hadBlocker, git) => {
            ...
            err(`  ${c.green('✓')} ${c.dim(rpad('[' + id + ']', 10))} ${rpad(agent, 22)} ${title.slice(0, 44)}${blockerTag}`);
            if (stream && output.trim()) {
                ...
                if (output.length > 360)
                    err('    ' + c.dim('…(truncated — full output in synthesis)'));
                err('');
            }
        },
        ...
        onWaveComplete: (waveNumber, decision) => {
            ...
            if (decision.decision === 'continue') {
                ...
                if (decision.pmNotes) {
                    ...
                    for (const nl of noteLines.slice(1))
                        err(`           ${c.dim(nl)}`);
                }
                err('');
                for (const t of (decision.newTasks ?? [])) {
                    err(`  ${c.green('+')} spawn    ${c.cyan(rpad(t.id, 10))} → ${c.bold(rpad(t.agent, 20))}  ` +
                        c.dim('"' + t.title.slice(0, 52) + '"'));
                }
                for (const u of (decision.unblocks ?? [])) {
                    err(`  ${c.yellow('↑')} unblock  ${c.bold(rpad(u.forAgent, 20))}  ` +
...
//# sourceMappingURL=team-cli.js.map