...
function isImplementationAgent(name) {
    const n = name.toLowerCase();
    if (TEST_AUTHOR_AGENTS.has(n) || TEST_EXECUTOR_AGENTS.has(n))
        return false;
    return IMPLEMENTATION_AGENTS.has(n) || n.includes('executor');
}
function isTestAuthorAgent(name) {
    const n = name.toLowerCase();
    return TEST_AUTHOR_AGENTS.has(n) || n.includes('test-author');
}
function isTestExecutorAgent(name) {
 ...
 */
export function buildClaudeToolCallingPrompt(input) {
    ...
    const p = payload.success
        ? payload.data
        : { agentName: 'agent', taskContext: input.taskContext, tools: [] };
    const sections = [];
    // ── Role identity ─────────────────────────────────────────────────────────
    ...
    if (input.teamGoal || input.blackboardSnapshot || input.commandBlackboardSnapshot || input.teamSize) {
        const teamParts = [];
        if (input.teamSize) {
            teamParts.push(`You are one of **${input.teamSize} specialist agents** working in parallel on this team. ` +
            ...
            teamParts.push(`You are part of a specialist engineering team. A **Lead PM** assigned this task to you and will ` +
...
**If you are not blocked — omit these sections entirely.** Just deliver your work.`);
    // ── Response format ───────────────────────────────────────────────────────
    const producesDotGraph = toolsList.includes('dependency-mapper') || toolsList.includes('graph-visualizer');
