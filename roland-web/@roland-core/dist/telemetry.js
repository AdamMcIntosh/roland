...
export function hasConsent(scope = 'user') {
    ...
    try {
        ...
        return typeof raw === 'object' && raw !== null && 'consent' in raw && raw.consent === true;
    }
    catch {
        return false;
    }
...
/** Persist consent. */
export function setConsent(scope = 'user') {
    const p = getConsentPath(scope);
...
export function initTelemetry(options) {
    if (initialized) {
        if (process.env.RCO_VERBOSE !== '0')
            console.error(`${LOG_PREFIX} Already initialized`);
        return;
    }
    if (!hasConsent()) {
        if (process.env.RCO_VERBOSE !== '0')
            console.error(`${LOG_PREFIX} Skipping init: no consent`);
        return;
    }
    if (DSN === DSN_PLACEHOLDER) {
        if (process.env.RCO_VERBOSE !== '0')
            console.error(`${LOG_PREFIX} DSN placeholder; set SENTRY_DSN for real reporting`);
    }
    try {
        Sentry.init({
            dsn: DSN,
            ...
            beforeSend(event) {
                if (process.env.RCO_VERBOSE !== '0')
                    console.error(`${LOG_PREFIX} Sending event: ${event.exception?.values?.[0]?.type ?? 'session'}`);
                return event;
            },
        });
        initialized = true;
        if (process.env.RCO_VERBOSE !== '0')
            console.error(`${LOG_PREFIX} Initialized (opt-in)`);
    }
    catch (err) {
...
export function startSession() {
    ...
    try {
        Sentry.startSession();
        if (process.env.RCO_VERBOSE !== '0')
            console.error(`${LOG_PREFIX} Session started`);
    }
    catch {
        // ignore
    }
