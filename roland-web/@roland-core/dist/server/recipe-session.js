...
export class RecipeSessionManager {
    ...
    // --------------------------------------------------------------------------
    startSession(recipe, userTask, fileBundle) {
    ...
    // --------------------------------------------------------------------------
    advanceSession(sessionId, stepOutput, costData) {
        ...
        if (costData) {
            ...
            this.costTracker.recordCost(costData.model || 'unknown', 'cursor', agentName, stepCostEntry.input_tokens, stepCostEntry.output_tokens, stepCostEntry.cost, { query: `Recipe: ${session.recipe.name}, step ${session.currentStep}` });
        }
    ...
    // --------------------------------------------------------------------------
    buildStepPrompt(session) {
        const stepDef = session.recipe.steps[session.currentStep];
        const agentName = this.normalizeAgentName(stepDef.agent);
        // Find the subagent definition to get its prompt template
        const subagent = session.recipe.subagents.find((s) => this.normalizeAgentName(s.name) === agentName);
    ...
    buildSummary(session) {
        ...
        logger.info(`[RecipeSession] Session ${session.id} completed — ` +
    ...
    // --------------------------------------------------------------------------
    interpolateVariables(input, session) {
        let result = input;
        // Replace {{user_task}}
    ...
    interpolateAgentRefs(text, session) {
...
//# sourceMappingURL=recipe-session.js.map