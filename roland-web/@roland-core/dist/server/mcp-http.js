...
async function probeToolCount() {
    if (cachedToolCount !== null)
 ...
 */
export async function buildMcpDiscovery(options = {}) {
    ...
    return {
        ...
        instructions: 'Connect with an MCP Streamable HTTP client. POST an initialize request to open a session; ' +
...
export async function buildMcpHealth() {
    const toolsCount = await probeToolCount();
    return {
        status: 'healthy',
        server: 'roland',
...
export function matchMcpHttpPath(urlPath, basePath = '/mcp') {
    const normalized = urlPath.split('?')[0].replace(/\/$/, '') || '/';
    const base = basePath.replace(/\/$/, '') || '/mcp';
    if (normalized === `${base}/health`)
        return 'health';
    if (normalized === base || normalized === '/api/mcp')
        return 'mcp';
    return null;
}
