 ...
 */
function getBudgetDegradedModel(agentName) {
    ...
    const usagePercent = status.maxBudget > 0
        ...
        : 0;
    if (usagePercent < BUDGET_DEGRADATION_THRESHOLD)
        return null;
...
export class McpServer {
            ...
            if (isComplexExecution) {
                ...
                if (gatheringConfig.enabled) {
                    try {
                        ...
                        logger.warn(`[Triage] File gathering failed: ${err.message}`);
                    }
                }
     ...
     */
    buildTriageReasoning(topAgent, topRecipe, complexity, suggestRecipe) {
        const parts = [];
        if (topAgent.score > 0) {
    ...
    // --------------------------------------------------------------------------
    registerRouteModel() {
        this.registerTool('route_model', 'Analyze query complexity and recommend the cheapest adequate model. Call this before making an LLM request to optimize cost.', async (args) => {
            const query = args.query;
            if (!query) {
                throw new McpToolError('route_model', 'query is required');
            }
            const budgetHint = args.budget || 'moderate';
            ...
            const estimatedCost = ModelRouter.estimateCost(recommendedModel, analysis.tokenEstimate, analysis.tokenEstimate * 2 // Rough output estimate
            );
    ...
    // --------------------------------------------------------------------------
    registerTrackCost() {
        this.registerTool('track_cost', 'Log token usage from an LLM call and return session totals with budget status. Call this after each LLM interaction to track spending.', async (args) => {
    ...
    // manage_budget — get/set/reset spending limits
    // --------------------------------------------------------------------------
    registerManageBudget() {
        this.registerTool('manage_budget', 'Manage API spending budget — check status, set limits, or reset spending. Use this to enforce cost controls.', async (args) => {
            ...
            switch (action) {
                ...
                case 'enable': {
                    const maxBudget = args.daily_limit;
    ...
    registerGetAnalytics() {
        this.registerTool('get_analytics', 'Get cost and token usage analytics for the current session, grouped by model, agent, or provider.', async (args) => {
            ...
            // Quality analytics
            const allQuality = this.qualityTracker.getModelQuality();
            let qualityRecommendation = null;
    ...
    // --------------------------------------------------------------------------
    registerSuggestMode() {
        this.registerTool('suggest_mode', 'Analyze a task and suggest the appropriate depth level (quick/standard/deep) with recommended agent chain. Use this to decide how much effort to invest in a task.', async (args) => {
            const query = args.query;
            if (!query) {
    ...
    // --------------------------------------------------------------------------
    registerListRecipes() {
        this.registerTool('list_recipes', 'List all available multi-agent workflow recipes with their descriptions and agent chains', async () => {
    ...
    scanRecipeFiles() {
        ...
        for (const file of files) {
            try {
                ...
                if (!raw)
                    continue;
                const agents = (raw.subagents || []).map((s) => s.name || 'unknown');
                results.push({
    ...
    // --------------------------------------------------------------------------
    registerStartRecipe() {
        this.registerTool('start_recipe', 'Start a multi-agent recipe session. Returns the first step\'s system prompt and user prompt for you to execute. Then call advance_recipe with your output to get the next step. Available recipes: BugFix, RESTfulAPI, SecurityAudit, WebAppFullStack, MicroservicesArchitecture, PlanExecRevEx, DocumentationRefactor, DesktopApp, VB6Migration.', async (args) => {
            const recipeName = args.recipe_name;
    ...
    // advance_recipe — submit step output, get next step or summary
    // --------------------------------------------------------------------------
    registerAdvanceRecipe() {
        this.registerTool('advance_recipe', 'Submit the output from the current recipe step and get the next step\'s prompt. When all steps are complete, returns a summary. Pass cost data if available for budget tracking.', async (args) => {
            const sessionId = args.session_id;
            const stepOutput = args.step_output;
            if (!sessionId) {
            ...
            const costData = args.cost ? args.cost : undefined;
            try {
    ...
    registerSessionContext() {
        this.registerTool('session_context', 'Persistent memory for long coding sessions. Tracks decisions, file changes, patterns, migration progress, and errors across subagent calls. Use this to maintain context continuity — call "get" before spawning subagents and "update" after each step.', async (args) => {
            ...
            switch (action) {
                ...
                case 'update': {
                    ...
                    if (args.file_change)
                        updates.file_change = args.file_change;
                    if (args.pattern)
                        updates.pattern = args.pattern;
                    if (args.migration)
                        updates.migration = args.migration;
                    if (args.error_resolved)
                        updates.error_resolved = args.error_resolved;
                    if (args.note)
                        updates.note = args.note;
                    if (args.advance_step !== undefined)
                        updates.advance_step = args.advance_step;
    ...
    registerProjectContext() {
        this.registerTool('project_context', 'Persistent cross-session knowledge base. Compounds conventions, patterns, decisions, and error resolutions over time. Entries gain confidence with repeated observation and stale entries are pruned automatically.', async (args) => {
            ...
            switch (action) {
                ...
                case 'observe': {
                    ...
                    if (args.examples)
                        data.examples = args.examples;
                    if (args.rationale)
                        data.rationale = args.rationale;
                    if (args.error_pattern)
                        data.error_pattern = args.error_pattern;
                    if (args.resolution)
                ...
                case 'pin': {
                    const id = args.id;
                    if (!id)
                        throw new McpToolError('project_context', 'id is required for pin action');
                ...
                case 'unpin': {
                    const id = args.id;
                    if (!id)
                        throw new McpToolError('project_context', 'id is required for unpin action');
                ...
                case 'remove': {
                    const id = args.id;
                    if (!id)
                        throw new McpToolError('project_context', 'id is required for remove action');
    ...
    // --------------------------------------------------------------------------
    registerQualitySignal() {
        this.registerTool('quality_signal', 'Record quality feedback for a model response. Call after each agent response with accept/retry/reject/manual_fix to help Roland learn which models work best for your codebase.', async (args) => {
            const model = args.model;
            ...
            const task_type = args.task_type || 'unknown';
            const complexity_tier = args.complexity_tier || 'unknown';
            const retry_model = args.retry_model;
            await this.qualityTracker.recordSignal(model, provider, task_type, complexity_tier, signal, retry_model);
    ...
    // Parse a loaded Recipe into the session manager's ParsedRecipe format
    // --------------------------------------------------------------------------
    parseRecipeForSession(recipeData) {
        ...
        const rawSteps = recipeData.steps ||
            ...
            [];
        const subagents = rawSubagents.map((sa) => ({
    ...
    // --------------------------------------------------------------------------
    registerLoadMigrationContext() {
        this.registerTool('load_migration_context', 'Load the project migration context (roland-context.json + .rco-state.json) and return a prompt-ready markdown block. Call this at the start of every session to inject mapping rules, past decisions, and test patterns. Optionally initialise a new session ID.', async (args) => {
    ...
    // --------------------------------------------------------------------------
    registerUpdateMigrationContext() {
        this.registerTool('update_migration_context', 'Append a new mapping rule, architectural decision, test pattern, or custom section to roland-context.json and regenerate MIGRATION.md. Use this whenever a new VB6→C# pattern or project decision is discovered.', async (args) => {
            const type = args.type;
            const projectRoot = typeof args.project_root === 'string' && args.project_root
                ? args.project_root
                : undefined;
            if (!type)
                throw new McpToolError('update_migration_context', '"type" is required');
            switch (type) {
                case 'rule': {
                    ...
                    const rule = appendRule(pattern, replacement, args.notes, projectRoot);
                ...
                case 'test_pattern': {
                    ...
                    const patternDescription = args.description;
                    ...
                    const tp = appendTestPattern(name, patternDescription, args.example, projectRoot);
    ...
    registerPreviewChanges() {
        this.registerTool('preview_changes', 'Generate a markdown unified diff and optional HTML preview comparing original vs modified content. Returns diff stats (additions/deletions) alongside formatted output.', async (args) => {
    ...
    registerGitTools() {
        ...
        this.registerTool('git_commit', 'Stage files and create a git commit. Pass files[] to stage specific paths, or omit to stage all changes (git add -A).', async (args) => {
            ...
            const message = args.message;
            if (!message)
                throw new McpToolError('git_commit', 'message is required');
    ...
    // --------------------------------------------------------------------------
    registerAnalyzeScreenshot() {
        this.registerTool('analyze_screenshot', 'Capture the primary screen (or load an existing image file) and analyse it with a vision-capable model. Returns a text description of what is visible — code, errors, UI, etc. Useful when debugging visual issues or reading screenshots.', async (args) => {
            const filePath = typeof args.file_path === 'string' ? args.file_path : undefined;
    ...
    // --------------------------------------------------------------------------
    registerReadContext() {
        this.registerTool('read_context', 'Read file contents from the project codebase. Use this when you need additional files beyond what was provided in relevant_files. Pass an array of file paths to read. Returns file contents ready to use as context for code generation.', async (args) => {
    ...
    registerCoordinationTools() {
        ...
        this.registerTool('bus_poll', 'Drain undelivered messages addressed to an agent (directly or via broadcast). By default acknowledges them so they are not returned again. Returns messages oldest-first plus a nextSince cursor for the next poll.', async (args) => {
            ...
            const nextSince = messages.length > 0 ? messages[messages.length - 1].ts + 1 : args.since;
    ...
    registerPmTools() {
        ...
        this.registerTool('get_pm_events', 'The PM event timeline: a reverse-chronological audit trail of lifecycle actions (spawn/assign/block/unblock/complete/review/usage/recipe-start) from .roland/pm-events.log. Use this to answer "what happened on this feature?". Pass format:"markdown" for a rendered timeline.', async (args) => {
            ...
            if (args.format === 'markdown')
                return { markdown: renderTimeline(events), events };
            return { events };
        }, {
            type: 'object',
            properties: {
