...
export function writeSyncState(state, cwd = process.cwd()) {
    const p = getSyncStatePath(cwd);
 ...
 */
export async function pushToRemote(_remote, _paths, _cwd) {
    if (process.env.RCO_VERBOSE !== '0') {
 ...
 */
export async function pullFromRemote(_remote, _cwd) {
    if (process.env.RCO_VERBOSE !== '0') {
