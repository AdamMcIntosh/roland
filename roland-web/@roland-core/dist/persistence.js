...
import { PersistedStateSchema } from './schemas.js';
const RCO_VERBOSE = process.env.RCO_VERBOSE !== '0' && process.env.RCO_VERBOSE !== 'false';
function log(msg) {
    if (RCO_VERBOSE)
        console.error(`[RCO persistence] ${msg}`);
}
 ...
 */
export function buildNotepadStorePrompt(state, key) {
 ...
 */
export function buildNotepadRetrievePrompt(sessionId) {
 ...
 */
export function parseNotepadResponse(raw) {
 ...
 */
export function saveStateToLocal(state, stateDir = DEFAULT_STATE_DIR) {
 ...
 */
export function loadStateFromLocal(sessionId, stateDir = DEFAULT_STATE_DIR) {
    const dir = path.isAbsolute(stateDir) ? stateDir : path.join(process.cwd(), stateDir);
    ...
    try {
        ...
        log(`Load failed ${filePath}: ${e.message}`);
        return null;
    }
}
/**
 ...
 */
export function listLocalSessionIds(stateDir = DEFAULT_STATE_DIR) {
    const dir = path.isAbsolute(stateDir) ? stateDir : path.join(process.cwd(), stateDir);
