...
// ── Step 3: OpenRouter API key prompt ─────────────────────────────────────────
...
function mergeYamlKey(existing: string, key: string, value: string): string {
  ...
  // We handle the specific shape: budget:\n  openrouter_api_key: <value>
...
function printSummary(apiKey: string | null): void {
  ...
  log('  3. Run a team mission for multi-step work:');
  log('     ' + chalk.cyan('roland team "your goal" --loop-template full-cycle-verified-loop'));
  log('');
