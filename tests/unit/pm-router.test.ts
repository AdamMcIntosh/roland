...
describe('modelForLane', () => {
  it('maps lanes to Cursor models under the default policy', () => {
    expect(DEFAULT_MODEL_POLICY.pm).toBe('grok-4.5');
    expect(modelForLane('pm').model).toBe('grok-4.5');
...
describe('TaskRouter', () => {
  ...
  it('routes reasoning roles to composer-2.5 (interactive)', () => {
    const d = legacyRouter.route('review the implementation', 'code-reviewer');
  ...
  it('routes execution roles to composer-2.5 (background)', () => {
    const d = legacyRouter.route('implement the feature', 'executor');
    expect(d.model).toBe('composer-2.5');
  ...
  it('never emits an OpenRouter slug or a free-tier model', () => {
    const router = new TaskRouter({ policy: DEFAULT_MODEL_POLICY });
