...
describe('stripMissionNoise', () => {
  ...
  it('removes inline mission tags after task prefix stripping', () => {
    expect(stripMissionNoise('Task task-2: [Mission: mobile-first] Fix layout')).toBe('Fix layout');
    expect(stripMissionNoise('Implement [Mission: x] feature end-to-end')).toBe('Implement feature end-to-end');
  });
...
describe('buildPrDescription', () => {
  ...
  it('applies safe defaults for empty task description', () => {
    ...
    expect(body).toMatch(/## Key changes\nChore bump/);
